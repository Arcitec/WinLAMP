...
static BOOL CALLBACK whatsnewProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if(uMsg == fm_msg)
	{
		...
		if(lpFindReplace->Flags & FR_FINDNEXT)
		{
			...
			{
				MessageBoxW(finder,getStringW(IDS_NO_MATCHES_FOUND,NULL,0),L"WinLAMP",MB_ICONINFORMATION);
			}
...
/* This does the 'zooming' effect on the image in the 'winlamp' tab of the about screen */
class AboutContext
{
public:
...
/* Window Proc for the 'winlamp' tab of the about screen */
	...
	case WM_INITDIALOG:
		{
			...
			static char ascii[]="\n\n  ______  ____  ___/\\  _/\\_ _____  dro  _____   <3  ______  /\\____   _______\n\
													...
													/    /     /     /     /          \\/   |/     \\/    /  /  /     /   |/    /\n\
 													...
 													(_________________________________________________________________\\\\\\_WINLAMP_)\n\n\n\
		...
		switch (LOWORD(wParam))
		{
			case IDC_WINLAMPLINK:
			{
				...
				if (homepage[0])
					...
					myOpenURL(hwndDlg,L"http://www.winlamp.com/");
			}
			return 0;
		}
	}
