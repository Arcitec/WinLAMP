...
void WADlg_init(HWND hwndWinLAMP); // call this on init, or on WM_DISPLAYCHANGE
int WADlg_initted();
void WADlg_close();
...
void WADlg_init(HWND hwndWinLAMP) // call this on init, or on WM_DISPLAYCHANGE
{
	...
	wadlg_bitmap = (HBITMAP) SendMessage(hwndWinLAMP,WM_WA_IPC,0,IPC_GET_GENSKINBITMAP);
	if (wadlg_bitmap)
	{
