...
static int EQ_OnLButtonDblClk(HWND hwnd, BOOL fDoubleClick, int x, int y, UINT keyFlags)
{
	...
	if (y <= 14 && x < 252)
	{
		SendMessageW(hMainWindow, WM_COMMAND, WINLAMP_OPTIONS_WINDOWSHADE_EQ, 0);
	}
	return 1;
}
...
static char sig[] = "WinLAMP EQ library file v1.1\x1A!--";
...
void eq_dialog(HWND hwnd, int init_state)
{
	...
	if (!config_eq_open)
	{
		CheckMenuItem(main_menu, WINLAMP_OPTIONS_EQ, MF_CHECKED);
		if (eq_startuphack)
		{
	...
	else
	{
		...
		CheckMenuItem(main_menu, WINLAMP_OPTIONS_EQ, MF_UNCHECKED);
