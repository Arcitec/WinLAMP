...
BOOL InitApplication(HINSTANCE hInstance)  // register WinLAMP's window class, returns TRUE on success
{
	HICON hIcon;
	WNDCLASSW wc = {0, };
	...
	//wc.lpszClassName = L"WinLAMP";;
	//if (!RegisterClassW( &wc ))
	//	return 0;

	wc.lpfnWndProc   = VIS_WndProc;
	wc.lpszClassName = L"WinLAMPVis";
	if (!RegisterClassW(&wc))
		return 0;


	wc.lpfnWndProc   = video_WndProc;
	wc.lpszClassName = L"WinLAMP Video";
	if (!RegisterClassW(&wc))
		return 0;
	...
	wcW.lpszClassName = L"WinLAMP Gen";
	if (!RegisterClassW(&wcW))
		return 0;

	wcW.style        |= CS_VREDRAW | CS_HREDRAW;
	wcW.lpfnWndProc   = EQ_WndProc;
	wcW.lpszClassName = L"WinLAMP EQ";
	if (!RegisterClassW(&wcW))
		return 0;

	wcW.lpfnWndProc   = PE_WndProc;
	wcW.lpszClassName = L"WinLAMP PE";
	if (!RegisterClassW(&wcW))
		return 0;

	// make other people's dialogs show the winlamp icon
...
void main_Create()
{
	...
	hVisWindow = CreateWindowExW(0, L"WinLAMPVis", L"", WS_CHILD, 0, 0, 0, 0,
	...
	top_menu = LPLoadMenu(WINLAMP_MAIN);

#ifdef NETSCAPE
	EnableMenuItem(top_menu, WINLAMP_OPTIONS_MINIBROWSER, MF_BYCOMMAND | MF_GRAYED);
#endif
	...
	draw_setnoupdate(0);
	InsertMenuW(GetSystemMenu(hMainWindow, FALSE), 0, MF_POPUP | MF_BYPOSITION, (UINT_PTR) main_menu, L"WinLAMP");
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow) // create the main WinLAMP window
{
	...
	if (!CreateWindowExW(0, L"WinLAMP EQ", getStringW(IDS_EQCAPTION, NULL, 0), WS_OVERLAPPED,
	                    config_eq_wx, config_eq_wy, 0, 0,  // WM_CREATE will size it
	                    hMainWindow, NULL, hInstance, NULL)) return FALSE;
	
	if (!CreateWindowExW(WS_EX_ACCEPTFILES, L"WinLAMP PE", getStringW(IDS_PECAPTION, NULL, 0), WS_OVERLAPPED,
	...
	hPLVisWindow = CreateWindowExW(0, L"WinLAMPVis", L"", WS_CHILD,
	                              0, 0, 0, 0,
	                              hPLWindow, NULL, hMainInstance, NULL);
	...
	if (!config_mw_open)
	{
		...
		//config_mw_open=1;
		//SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_MAIN_WINDOW,0);
	}
	else
		CheckMenuItem(main_menu, WINLAMP_MAIN_WINDOW, MF_CHECKED);
...
*/
	...
	v5_top_menu = LPLoadMenu(WINLAMP5_MENUBAR);
