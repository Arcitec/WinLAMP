...
CommonReader *DecodeFile::MakeReader(const wchar_t *filename, AudioParameters *parameters, bool useUnagi)
{
	...
	while (a >= 0)
	{
		...
		open = (OpenFunc)GetProcAddress(in->hDllInstance, "winlampGetExtendedRead_open");
		openFloat = (OpenFunc)GetProcAddress(in->hDllInstance, "winlampGetExtendedRead_open_float");
		openW = (OpenWFunc)GetProcAddress(in->hDllInstance, "winlampGetExtendedRead_openW");
		openWFloat = (OpenWFunc)GetProcAddress(in->hDllInstance, "winlampGetExtendedRead_openW_float");
		getData = (GetDataFunc)GetProcAddress(in->hDllInstance, "winlampGetExtendedRead_getData");
		close = (CloseFunc)GetProcAddress(in->hDllInstance, "winlampGetExtendedRead_close");
		setTime = (SetTimeFunc)GetProcAddress(in->hDllInstance, "winlampGetExtendedRead_setTime"); //optional!
