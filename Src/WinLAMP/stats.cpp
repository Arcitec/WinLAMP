...
void Stats::Init()
{
	...
	char *p=str;
	GetPrivateProfileStringA("WinLAMPReg","Stats","",str,sizeof(str),INI_FILEA);
...
void Stats::Write()
{
	...
	WritePrivateProfileStringA("WinLAMPReg","Stats",str,INI_FILEA);
}
...
{
	WritePrivateProfileStringA("WinLAMPReg",key,AutoChar(value, CP_UTF8),INI_FILEA);
}
...
void Stats::GetString(const char *key, wchar_t *value, size_t value_cch) const
	{
		*value = 0;
		...
		if (utf8)
		{
			GetPrivateProfileStringA("WinLAMPReg",key,"",utf8,(DWORD)value_cch,INI_FILEA);
...
{
	GUID uid;
	GetPrivateProfileStringA("WinLAMPReg","ID","",str,128,INI_FILEA);
	...
	if (!str[0])
	{
		...
		WritePrivateProfileStringA("WinLAMPReg","ID",str,INI_FILEA);
	}
}
