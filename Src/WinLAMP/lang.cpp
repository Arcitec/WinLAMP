...
static std::vector<winlampLangStruct*> lnglist;
...
int LPMessageBox(HWND parent, UINT idMessage, UINT idTitle, UINT type)
{
	...
	// TODO consider exposing something in the winlamp.lng file so we can follow this where possible as it should allow for a localised messagebox as long as the OS supports the language
...
const wchar_t *Language::GetLanguageIdentifier( int mode )
{
	...
	// make sure to try to use a loaded winlamp.lng as load_extra_lng(..) relies on
	// this for the path to use but calls it before getStringW(..) will work fully
	GetStringFromGUIDW( WinLAMPLangGUID, hMainInstance, LANG_PACK_LANG_ID, id_str, 9 );
...
// attempt to cleanup the last extracted temp folder for a wlz incase WinLAMP crashed on exit
		...
		if (lang_identifier || force)
		{
			...
			if (h != INVALID_HANDLE_VALUE)
			{
				do
				{
					...
					is_wlz = 1;
					winlampLangStruct* templng = reinterpret_cast<winlampLangStruct*>(calloc(1, sizeof(winlampLangStruct)));
...
HINSTANCE Language::FindDllHandleByGUID(const GUID guid)
{
	...
	for ( winlampLangStruct *l_lng : lnglist )
	{
...
HINSTANCE Language::StartLanguageSupport(HINSTANCE hinstance, const GUID guid)
{
	if (!g_safeMode)
	{
		HWND agent = FindWindowW(L"WinLAMPAgentMain", NULL);
		wchar_t winlampaLngPath[MAX_PATH] = {0};
		int is_wlz = 0;

		// if we find WinLAMP Agent running then we need to tell it
		// to unload it's winlampa.lng for what we're about to do..
		if (IsWindow(agent) && !already_extracted)
		{
			SendMessageW(agent, WM_USER + 16, 1, 0);
		}

		// always remove winlampa.lng just incase we crashed and it leaves things out of synch
		if(!already_extracted){
			StringCchPrintfW(winlampaLngPath, MAX_PATH, L"%s\\winlampa.lng", CONFIGDIR);
			DeleteFileW(winlampaLngPath);
		}
		...
		if (config_langpack[0] && is_wlz)
		{
			...
			if (h)
			{
				...
				// then we see if WinLAMP Agent is running and tell it to refresh
				// it's version of winlampa.lng once we've copied into %inidir%
				if (IsWindow(agent))
				{
					// copy from the wlz folder to the settings folder
					wchar_t winlampaWlzPath[MAX_PATH] = {0};
					StringCchPrintfW(winlampaWlzPath, MAX_PATH, L"%s\\winlampa.lng", lang_directory);
					CopyFileW(winlampaWlzPath,winlampaLngPath,FALSE);
...
void Lang_EndLangSupport(void)
{
	...
	for ( winlampLangStruct *l_lng : lnglist )
	{
		...
		// this check is to prevent trying to re-free the winlamp.lng (as it's done earlier)
...
HINSTANCE Lang_FakeWinLAMPLangHInst(HINSTANCE adjustedHInst){
...
void Lang_LocaliseAgentOnTheFly(BOOL refresh){
	// if we need to refresh then attempt to use the winlampa.lng from the
