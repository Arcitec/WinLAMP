...
extern std::vector<winlampGeneralPurposePlugin*> gen_plugins;
// gen tab procedure
...
INT_PTR CALLBACK GenProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	else if (uMsg == WM_COMMAND)
	{
		switch (LOWORD(wParam))
		{
					...
					if (IsWindowEnabled(GetDlgItem(hwndDlg, IDC_UNINST)))
					{
							...
							LPMessageBox(hwndDlg, IDS_P_PLUGIN_UNINSTALL,IDS_P_PLUGIN_UNINSTALL_CONFIRM, MB_YESNO | MB_ICONEXCLAMATION) == IDYES)
						{
							if (lvi.lParam >= 0 && lvi.lParam < (int)gen_plugins.size())
							{
								...
								if (lvi.lParam != -2)
								{
									*(void**)&pr = (void*)GetProcAddress(gen_plugins[lvi.lParam]->hDllInstance, "winlampUninstallPlugin");
			...
			case IDC_PLUGINVERS:
				myOpenURLWithFallback(hwndDlg, L"http://www.google.com/search?q=WinLAMP+General+Plugins", L"http://www.google.com/search?q=WinLAMP+General+Plugins");
				break;
		}
	}
	else
