	...
	if (format->out && format->out_len)
	{
		...
		if (WINLAMP5_API_TAGZ)
		{
			if (format->TAGFUNC)
			{
				...
				if (tempOut)
				{
					WINLAMP5_API_TAGZ->format(spec, tempOut, format->out_len, &provider, 0 /*&parameters*/);
			...
			else
			{
				...
				if (tempOut)
				{
					WINLAMP5_API_TAGZ->format(spec, tempOut, format->out_len, tagProvider, &parameters);
					WideCharToMultiByteSZ(CP_ACP, 0, tempOut, -1, format->out, format->out_len, 0, 0);
					free(tempOut);
				}
			}
...
void FormatTitleExtended(waFormatTitleExtended *format)
{
	...
	if (format->out && format->out_len)
	{
		if (WINLAMP5_API_TAGZ)
		{
			...
			WINLAMP5_API_TAGZ->format(spec, format->out, format->out_len, &provider, 0 /*&parameters*/);
		...
		((hts->force_useformatting&1) || !(PathIsURLW(hts->filename) && StrCmpNIW(hts->filename, L"cda://", 6))))
	{
			...
			in_get_extended_fileinfoW(hts->filename, L"type", buf2, 32))
		{
			...
			if (WINLAMP5_API_TAGZ)
			{
				...
				WINLAMP5_API_TAGZ->format(config_titlefmt, hts->title, 2048, tagProvider, &parameters);
			}
...
wchar_t *ExtendedTagProvider::GetTag(const wchar_t *name, ifc_tagparams *parameters)
{
	...
	{
		tag = WinLAMP::GetTag(name, filename);
		if (tag == reinterpret_cast<wchar_t *>(-1))
			return 0;
		if (!tag && useExtendedInfo)
			tag = WinLAMP::GetExtendedTag(name, filename);
		if (tag == reinterpret_cast<wchar_t *>(-1))
			return 0;
		return tag;		
	}
