/*
** Copyright (C) 1997-2014 WinLAMP SA
**
...
** result = SendMessageW(hwnd_winlamp,WM_WA_IPC,(parameter),IPC_*);
** Where different then this is specified (typically with WM_COPYDATA variants)
**
** When you use SendMessageW(hwnd_winlamp,WM_WA_IPC,(parameter),IPC_*) and specify a IPC_*
** which is not currently implemented/supported by the WinLAMP version being used then it
...
#define WINLAMP_VERSION_MAJOR(winlampVersion) ((winlampVersion & 0x0000FF00) >> 12)
#define WINLAMP_VERSION_MINOR(winlampVersion) (winlampVersion & 0x000000FF)  // returns, i.e. 0x12 for 5.12 and 0x10 for 5.1... 


#define IPC_GETVERSION 0
/* int version = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETVERSION);
**
** The version returned will be 0x20yx for WinLAMP 2.yx.
** Versions previous to WinLAMP 2.0 typically (but not always) use 0x1zyx for 1.zx.
** Just a bit weird but that's the way it goes.
**
** For WinLAMP 5.x it uses the format 0x50yx for WinLAMP 5.yx
** e.g.   5.01 -> 0x5001
**        5.09 -> 0x5009
**        5.1  -> 0x5010
**
...
/* (requires WinLAMP 5.0+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETREGISTEREDVERSION);
**
** This will open the WinLAMP Preferences and show the WinLAMP Pro page.
*/


#define IPC_IS_SAFEMODE 999
/* (requires WinLAMP 5.64+)
** int res = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_IS_SAFEMODE);
**
...
**   SendMessageW(hwnd_winlamp,WM_COPYDATA,0,(LPARAM)&cds);
**
**
...
**   SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)&eFWMS,IPC_ENQUEUEFILE);
*/
...
** Use this api to clear WinLAMP's internal playlist.
** You should not need to use IPC_DELETE_INT since it is used internally by WinLAMP when
...
**   SendMessageW(hwnd_winlamp,WM_COPYDATA,0,(LPARAM)&cds);
**
** The above example will make WinLAMP change to the directory 'C:\download'.
*/


#define IPC_ISPLAYING 104
/* int res = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_ISPLAYING);
** This is sent to retrieve the current playback state of WinLAMP.
** If it returns 1, WinLAMP is playing.
** If it returns 3, WinLAMP is paused.
** If it returns 0, WinLAMP is not playing.
*/


#define IPC_GETOUTPUTTIME 105
/* int res = SendMessageW(hwnd_winlamp,WM_WA_IPC,mode,IPC_GETOUTPUTTIME);
...
** Will return -1 if WinLAMP is not playing.
**
** If mode = 1 then it will return the current track length (in seconds).
** Will return -1 if there are no tracks (or possibly if WinLAMP cannot get the length).
**
...
/* (requires WinLAMP 1.60+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,ms,IPC_JUMPTOTIME);
...
** This returns -1 if WinLAMP is not playing, 1 on end of file, or 0 if it was successful.
*/
...
/* (requires WinLAMP 1.666+)
** int cur = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_WRITEPLAYLIST);
**
** IPC_WRITEPLAYLIST will write the current playlist to winlamp.m3u and on unicode clients
** it will also write the current playlist to winlamp.m3u8. This will also return the
** current playlist position (see IPC_GETLISTPOS).
**
** Where the playlist(s) are saved to depends on the WinLAMP client version used though it
** is simple to find the path using the correct location (when working in-process):
**
** Pre 2.90 -> '<winlampdir>\\WinLAMP.m3u'
...
** behaviour is what will be attempted to be used (as WinLAMP does if there is any failure).
**
...
** current playlist.
**
** This api will only save out extended file information in the #EXTINF entry if WinLAMP
** has already read the data such as if the file was played of scrolled into view. If
** WinLAMP has not read the data then you will only find the file with its filepath entry
** (as is the base requirements for a m3u playlist).
*/


#define IPC_SETPLAYLISTPOS 121
/* (requires WinLAMP 2.0+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,position,IPC_SETPLAYLISTPOS)
...
**
** If you use SendMessageW(hwnd_winlamp,WM_COMMAND,MAKEWPARAM(WINLAMP_BUTTON2,0),0);
** after using IPC_SETPLAYLISTPOS then WinLAMP will start playing the file at 'position'.
*/


#define IPC_SETVOLUME 122
/* (requires WinLAMP 2.0+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,volume,IPC_SETVOLUME);
** IPC_SETVOLUME sets the volume of WinLAMP (between the range of 0 to 255).
**
** If you pass 'volume' as -666 then the message will return the current volume.
** int curvol = SendMessageW(hwnd_winlamp,WM_WA_IPC,-666,IPC_SETVOLUME);
*/


#define IPC_GETVOLUME(hwnd_winlamp) SendMessageW(hwnd_winlamp,WM_WA_IPC,-666,IPC_SETVOLUME)
/* (requires WinLAMP 2.0+)
** int curvol = IPC_GETVOLUME(hwnd_winlamp);
** This will return the current volume of WinLAMP (between the range of 0 to 255).
*/
...
/* (requires WinLAMP 2.0+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,panning,IPC_SETPANNING);
** IPC_SETPANNING sets the panning of WinLAMP from 0 (left) to 255 (right).
**
...
** int curpan = SendMessageW(hwnd_winlamp,WM_WA_IPC,-666,IPC_SETPANNING);
*/


#define IPC_GETPANNING(hwnd_winlamp) SendMessageW(hwnd_winlamp,WM_WA_IPC,-666,IPC_SETPANNING)
#define IPC_GETBALANCE(hwnd_winlamp) SendMessageW(hwnd_winlamp,WM_WA_IPC,-666,IPC_SETBALANCE)
/* (requires WinLAMP 2.0+)
** int curpan = IPC_GETPANNING(hwnd_winlamp);
** This will return the current panning level of WinLAMP (5.x) from -127 (left) to 127 (right)
...
/* (requires WinLAMP 2.0+)
** int length = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETLISTLENGTH);
** IPC_GETLISTLENGTH returns the length of the current playlist as the number of tracks.
*/


#define IPC_GETLISTPOS 125
/* (requires WinLAMP 2.05+)
** int pos=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETLISTPOS);
...
/* (requires WinLAMP 5.61+)
** int pos=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETNEXTLISTPOS);
...
/* (requires WinLAMP 2.05+)
** int inf=SendMessageW(hwnd_winlamp,WM_WA_IPC,mode,IPC_GETINFO);
...
/* (requires WinLAMP 2.05+)
** int data=SendMessageW(hwnd_winlamp,WM_WA_IPC,pos,IPC_GETEQDATA);
...
/* (requires WinLAMP 2.05+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,pos,IPC_GETEQDATA);
** SendMessageW(hwnd_winlamp,WM_WA_IPC,value,IPC_SETEQDATA);
...
** file / url to the WinLAMP bookmark list.
**
** COPYDATASTRUCT cds = {0};
...
**
**
** In WinLAMP 5.0+ we use this as a normal WM_WA_IPC and the string is null separated as
** the filename and then the title of the entry.
**
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(char*)"filename\0title\0",IPC_ADDBOOKMARK);
**
...
** Passing wParam as 666 will return the winlamp.bm8 filepath and is valid for both apis.
*/
...
#define IPC_RESTARTWINLAMP 135
/* (requires WinLAMP 2.2+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_RESTARTWINLAMP);
** IPC_RESTARTWINLAMP will restart WinLAMP (isn't that obvious ? :) )
** If this fails to make WinLAMP start after closing then there is a good chance one (or
** more) of the currently installed plugins caused WinLAMP to crash on exit (either as a
** silent crash or a full crash log report before it could call itself start again.
*/


#define IPC_RESTARTSAFEWINLAMP 1135
/* (requires WinLAMP 5.64+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_RESTARTSAFEWINLAMP);
** IPC_RESTARTSAFEWINLAMP will restart WinLAMP (isn't that obvious ? :) ) in safe mode.
** If this fails to make WinLAMP start after closing then there is a good chance one (or
** more) of the currently installed plugins caused WinLAMP to crash on exit (either as a
** silent crash or a full crash log report before it could call itself start again.
*/
...
/* (requires winlamp 2.7+ I think)
** int ret=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_ISFULLSTOP);
...
/* (requires WinLAMP 2.05+)
** int val=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_INETAVAILABLE);
** IPC_INETAVAILABLE will return 1 if an Internet connection is available for WinLAMP and
** relates to the internet connection type setting on the main general preferences page
** in the WinLAMP preferences.
*/


#define IPC_UPDTITLE 243
/* (requires WinLAMP 2.2+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_UPDTITLE);
** IPC_UPDTITLE will ask WinLAMP to update the information about the current title and
...
/* (requires WinLAMP 2.2+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_REFRESHPLCACHE);
** IPC_REFRESHPLCACHE will flush the playlist cache buffer and you send this if you want
** WinLAMP to go refetch the titles for all of the entries in the current playlist.
**
** 5.3+:
...
/* (requires WinLAMP 2.4+)
** int val=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_SHUFFLE);
...
/* (requires WinLAMP 2.4+)
** int val=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_REPEAT);
...
/* (requires WinLAMP 2.4+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,value,IPC_SET_SHUFFLE);
...
/* (requires WinLAMP 2.4+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,value,IPC_SET_REPEAT);
...
/* (requires WinLAMP 2.9+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(enable?0:0xdeadbeef),IPC_ENABLEDISABLE_ALL_WINDOWS);
** Sending this message with 0xdeadbeef as the param will disable all winlamp windows and
** any other values will enable all of the WinLAMP windows again. When disabled you won't
** get any response on clicking or trying to do anything to the WinLAMP windows. If the
...
/* (requires WinLAMP 2.9+)
** HWND h=SendMessageW(hwnd_winlamp,WM_WA_IPC,IPC_GETWND_xxx,IPC_GETWND);
...
/* (requires WinLAMP 2.04+, only usable from plug-ins (not external apps))
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)"skinname",IPC_SETSKIN);
** IPC_SETSKIN sets the current skin to "skinname". Note that skinname 
** can be the name of a skin, a skin .zip file, with or without path. 
** If path isn't specified, the default search path is the winlamp skins 
** directory.
*/
...
/* (requires WinLAMP 2.04+, only usable from plug-ins (not external apps))
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)skinname_buffer,IPC_GETSKIN);
...
/* (requires WinLAMP 2.04+, only usable from plug-ins (not external apps))
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)"vis_file.dll",IPC_EXECPLUG);
** IPC_EXECPLUG executes a visualization plug-in pointed to by WPARAM.
** the format of this string can be:
** "vis_whatever.dll"
** "vis_whatever.dll,0" // (first mod, file in winlamp plug-in dir)
...
/* (requires WinLAMP 2.04+, only usable from plug-ins (not external apps))
** char *name=SendMessageW(hwnd_winlamp,WM_WA_IPC,index,IPC_GETPLAYLISTFILE);
...
/* (requires WinLAMP 2.04+, only usable from plug-ins (not external apps))
** char *name=SendMessageW(hwnd_winlamp,WM_WA_IPC,index,IPC_GETPLAYLISTTITLE);
**
...
/* (requires WinLAMP 2.05+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_MBOPEN);
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)url,IPC_MBOPEN);
...
/* (requires WinLAMP 2.05+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)file,IPC_CHANGECURRENTFILE);
** IPC_CHANGECURRENTFILE will set the current playlist item.
*/


#define IPC_CHANGECURRENTFILEW 1245
/* (requires WinLAMP 5.3+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)file,IPC_CHANGECURRENTFILEW);
...
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)buffer,IPC_GETMBURL);
...
/* (requires WinLAMP 2.4+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,value,IPC_MBBLOCK);
**
...
/* (requires WinLAMP 2.4+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)url,IPC_MBOPENREAL);
**
...
/* (requires WinLAMP 2.9+)
** int newpos=SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)adjust_offset,IPC_ADJUST_OPTIONSMENUPOS);
** This moves where WinLAMP expects the Options menu to be in the main menu.
...
/* (requires WinLAMP 2.9+)
** HMENU hMenu=SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)0,IPC_GET_HMENU);
**
...
** -1 : the main winlamp menu resource (same as doing LoadMenu(winlamp_module_or_lng_file,101)
**  0 : main popup menu 
**  1 : main menubar file menu
...
** HMENU windows_menu = (HMENU)SendMessageW(hwnd_winlamp,WM_WA_IPC,4,IPC_GET_HMENU);
** if(windows_menu)
** {
**   // WA_MENUITEM_ID is obtained from IPC_REGISTER_WINLAMP_IPCMESSAGE or a predefined
...
** length of that buffer, and then SendMessageW(hwnd_winlamp,WM_WA_IPC,&struct,IPC_GET_EXTENDED_FILE_INFO);
...
/* (requires WinLAMP 5.0+)
** char* ext_list = (char*)SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)mode,IPC_GET_EXTLIST);
** wchar_t* ext_list = (wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)mode,IPC_GET_EXTLISTW);
...
/* (requires WinLAMP 5.66+)
** wchar_t* pl_ext_list = (wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)mode,IPC_GET_PLAYLIST_EXTLISTW);
...
** int ret_val = SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)&infoBox,IPC_INFOBOX);
**
...
** int ret_val = SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)&infoBoxW,IPC_INFOBOXW);
**
** ret_val is 0 if OK was pressed and is 1 if CANCEL was pressed
** The standard behaviour is to abort any loops used with this api if a cancel occurs.
*/
...
** SendMessageW(hwnd_winlamp,WM_WA_IPC,&struct,IPC_SET_EXTENDED_FILE_INFO);
...
typedef struct 
{
  char *spec; // NULL=default winlamp spec
  void *p;
...
#define IPC_FORMAT_TITLE_EXTENDED 298 // similiar to IPC_FORMAT_TITLE, but falls back to WinLAMP's %tags% if your passed tag function doesn't handle it
typedef struct 
{
  const wchar_t *filename;
  ...
  const wchar_t *spec; // NULL=default winlamp spec
  void *p;
...
typedef struct _prefsDlgRec {
  ...
  intptr_t _id;
  struct _prefsDlgRec *next; // no longer implemented as a linked list, now used by WinLAMP for other means
} prefsDlgRec;
...
/* (requires WinLAMP 2.9+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)&prefsRec,IPC_ADD_PREFS_DLG);
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)&prefsRec,IPC_REMOVE_PREFS_DLG);
**
** IPC_ADD_PREFS_DLG:
...
**
** Then you do SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)&prefsRec,IPC_ADD_PREFS_DLG);
**
** example:
**
...
/* SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)&prefsRec,IPC_OPENPREFSTOPAGE);
**
...
** opened to the first page available (usually the WinLAMP Pro page).
**
** (requires WinLAMP 5.04+)
...
** created by the plugins shipped with WinLAMP.
*/


#define IPC_GETINIFILE 334
/* (requires WinLAMP 2.9+)
** char *ini=(char*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETINIFILE);
** This returns a pointer to the full file path of winlamp.ini.
**
** char ini_path[MAX_PATH] = {0};
**
** void GetIniFilePath(HWND hwnd){
**   if(SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETVERSION) >= 0x2900){
**     // this gets the string of the full ini file path
**     lstrcpynA(ini_path,(char*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETINIFILE),sizeof(ini_path)/sizeof(char));
**   }
**   else{
**   char* p = ini_path;
...
/* (requires WinLAMP 5.58+)
** wchar_t *ini=(wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETINIFILEW);
** This returns a pointer to the full file path of winlamp.ini.
**
** wchar_t ini_path[MAX_PATH] = {0};
**
** void GetIniFilePath(HWND hwnd){
**   if(SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETVERSION) >= 0x5058){
**     // this gets the string of the full ini file path
**     lstrcpynW(ini_path,(wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETINIFILEW),sizeof(ini_path)/sizeof(wchar_t));
**   }
**   else{
**   wchar_t* p = ini_path;
...
/* (requires WinLAMP 2.9+)
** char *dir=(char*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETINIDIRECTORY);
** This returns a pointer to the directory where winlamp.ini can be found and is
** useful if you want store config files but you don't want to use winlamp.ini.
*/


#define IPC_GETINIDIRECTORYW 1335
/* (requires WinLAMP 5.58+)
** wchar_t *dir=(wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETINIDIRECTORYW);
** This returns a pointer to the directory where winlamp.ini can be found and is
** useful if you want store config files but you don't want to use winlamp.ini.
*/
...
/* (requires WinLAMP 5.11+)
** char *plugdir=(char*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETPLUGINDIRECTORY);
** This returns a pointer to the directory where WinLAMP has its plugins stored and is
...
/* (requires WinLAMP 5.58+)
** wchar_t *plugdir=(wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETPLUGINDIRECTORYW);
** This returns a pointer to the directory where WinLAMP has its plugins stored and is
** useful if you want store config files in plugins.ini in the plugins folder or for
** accessing any local files in the plugins folder.
*/
...
/* (requires WinLAMP 5.11+)
** char *m3udir=(char*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETM3UDIRECTORY);
** This returns a pointer to the directory where winlamp.m3u (and winlamp.m3u8 if supported) is stored in.
*/


#define IPC_GETM3UDIRECTORYW 338
/* (requires WinLAMP 5.3+)
** wchar_t *m3udirW=(wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETM3UDIRECTORYW);
** This returns a pointer to the directory where winlamp.m3u (and winlamp.m3u8 if supported) is stored in.
*/
...
/* (requires WinLAMP 5.58+)
** wchar_t *visdir=(wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETVISDIRECTORYW);
** This returns a pointer to the directory set to load/store visualization plugins from.
**
** This can otherwise be obtained by reading the "VISDir" entry in the winlamp section of
** winlamp.ini which if null then you need to assume <winlampdir>\plugins
*/


#define IPC_GETSKINDIRECTORYW 340
/* (requires WinLAMP 5.58+)
** wchar_t *skindir=(wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETSKINDIRECTORYW);
** This returns a pointer to the directory set to load/store installed skins from.
**
** This can otherwise be obtained by reading the "SkinDir" entry in the winlamp section of
** winlamp.ini which if null then you need to assume <winlampdir>\skins
*/


#define IPC_GETDSPDIRECTORYW 341
/* (requires WinLAMP 5.58+)
** wchar_t *dspdir=(wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETDSPDIRECTORYW);
** This returns a pointer to the directory set to load/store installed DSP/effect plugins from.
**
** This can otherwise be obtained by reading the "DSPDir" entry in the winlamp section of
** winlamp.ini which if null then you need to assume <winlampdir>\plugins
*/
...
/* (requires WinLAMP 5.66+)
** wchar_t *ml_ini=(wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETMLINIFILEW);
** This returns a pointer to the full file path of gen_ml.ini.
*/


#define IPC_GETSHAREDDLLDIRECTORYW 1344
/* (requires WinLAMP 5.8+)
** wchar_t *shareddir=(wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETSHAREDDLLDIRECTORYW);
** This returns a pointer to the directory set to load/store installed DSP/effect plugins from.
*/
...
/* (requires WinLAMP 5.0+)
** HGLOBAL hglobal = (HGLOBAL)SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(HWND)parent,IPC_OPENURLBOX);
...
/* (requires WinLAMP 5.0+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(HWND)parent,IPC_OPENFILEBOX);
** You pass a hwnd for the dialog to be parented to (which modern skin support uses).
*/
...
/* (requires WinLAMP 5.0+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(HWND)parent,IPC_OPENDIRBOX);
** You pass a hwnd for the dialog to be parented to (which modern skin support uses).
*/
...
/* (requires WinLAMP 5.0+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(HWND)parent,IPC_SETDIALOGBOXPARENT);
** Pass 'parent' as the window which will be used as the parent for a number of the built
** in WinLAMP dialogs and is useful when you are taking over the whole of the UI so that
** the dialogs will not appear at the bottom right of the screen since the main winlamp
** window is located at 3000x3000 by gen_ff when this is used.  Call this again with
** parent = null to reset the parent back to the orginal WinLAMP window.
*/


#define IPC_GETDIALOGBOXPARENT 365
/* (requires WinLAMP 5.51+)
** HWND hwndParent = SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)0,IPC_GETDIALOGBOXPARENT);
** hwndParent can / must be passed to all modal dialogs/MessageBoxes when using WinLAMP as a parent.
*/


#define IPC_UPDATEDIALOGBOXPARENT 366
/* (requires WinLAMP 5.53+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(HWND)parent,IPC_UPDATEDIALOGBOXPARENT);
...
/* (requires WinLAMP 5.2+)
/* pass me an int (__cdecl *)(const char *, const char *) in wParam
** If this is not set or if it is cleared then WinLAMP reverts to it's own comparator.
** It is not until WinLAMP 5.33+ that passing wParam=0 or another function will allow
** for it to be changed from what was set on the first calling of this api.
*/

#define IPC_SET_JTF_COMPARATOR_W 410
/* (requires WinLAMP 5.55+)
** pass me an int (__cdecl *)(const wchar_t *, const wchar_t *) in wParam
** If this is not set or if it is cleared then WinLAMP will revert to the
...
/* (requires WinLAMP 5.66+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)mode,IPC_SET_JTF_LOAD_MODE);
...
** int mode = SendMessageW(hwnd_winlamp,WM_WA_IPC,-666,IPC_SET_JTF_LOAD_MODE);
*/

#define IPC_GET_JTF_LOAD_MODE(hwnd_winlamp) SendMessageW(hwnd_winlamp,WM_WA_IPC,-666,IPC_SET_JTF_LOAD_MODE)
/* (requires WinLAMP 5.66+)
** int curpan = IPC_GETPANNING(hwnd_winlamp);
...
typedef struct embedWindowState
{
  ...
  #pragma warning(pop)
  #pragma pack(pop)
  intptr_t extra_data[64];   // for internal winlamp use
  };
} embedWindowState;
...
/* (requires WinLAMP 2.9+)
** HWND myframe = (HWND)SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)&wa_wnd,IPC_GET_EMBEDIF);
**
** or
**
** HWND myframe = 0;
** HWND (*embed)(embedWindowState *params)=0;
**   *(void**)&embed = (void*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_EMBEDIF);
...
/* (requires WinLAMP 2.9+)
** int valid = SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)embedhwnd,IPC_EMBED_ISVALID);
...
/* (requires WinLAMP 5.66+)
** int done = SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(embedWindowState*)ws,IPC_EMBED_ADD_LEGACY);
...
/* (requires WinLAMP 5.66+)
** int done = SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(embedWindowState*)ws,IPC_EMBED_REMOVE_LEGACY);
** You pass an embedWindowState* filled with any appropriate details and ensure you add
** EMBED_FLAGS_LEGACY_WND to the 'flags' member to ensure it will be handled correctly.
** Returns 1 if the request was able to be processed.
*/
...
/* (requires WinLAMP 5.66+)
** int done = SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(embedWindowState*)ws,IPC_EMBED_UPDATE_LEGACY_POS);
** You pass an embedWindowState* filled with an updated position of the window by filling
** int the 'r' member so WinLAMP will know the correct position of the window after moving.
** Returns 1 if the request was able to be processed.
**
** Not sending this (ideally at the end of the window being moved) will likely cause issues
** with the placement and docking of the legacy window when WinLAMP's naitve and any embedded
...
**   *(void**)&SnapToScreen = (void*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_EMBED_SNAPFUNC);
**   SnapToScreen(&r);
**
** or
**
** SnapWindowToAllWindows is used to allow the window to natively dock to other WinLAMP windows
** RECT r;
** HWND myWnd = 0;
** void (*SnapWindowToAllWindows)(LPRECT outrc, HWND hwndNoSnap)=0;
**   *(void**)&SnapWindowToAllWindows = (void*)SendMessageW(hwnd_winlamp,WM_WA_IPC,1,IPC_GET_EMBED_SNAPFUNC);
**   SnapWindowToAllWindows(&r, myWnd);
*/


#define IPC_CONVERTFILE 506
/* (requires WinLAMP 2.92+)
...
**
** eg: SendMessageW(hwnd_winlamp,WM_WA_IPC,&myConvertStruct,IPC_CONVERTFILE);
**
** Return value:
...
typedef struct 
{
  ...
  int killswitch;     // don't set it manually, use IPC_CONVERTFILE_END
	...
	ifc_audiostream *decoder; // for internal winlamp use 
...
**
** eg: SendMessageW(hwnd_winlamp,WM_WA_IPC,&myConvertStruct,IPC_CONVERTFILE_END);
**
** No return value
*/
...
* checks: 
* - valid input file
* - valid destination format / encoder type (including winlamp pro authorization for mp3)
...
** {
**   LRESULT ret = CallWindowProc((WNDPROC)WinLAMPProc,hwnd,umsg,wParam,lParam);
**
...
/* (requires WinLAMP 5.0+)
** <returned_function_pointer> = SendMessageW(hwnd_winlamp,WM_WA_IPC,param,IPC_GETSADATAFUNC);
...
**
** When called this will return 150 bytes of data as a static buffer inside WinLAMP.
...
/* (requires WinLAMP 5.0+)
** int visible=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_ISMAINWNDVISIBLE);
** You send this to WinLAMP to query if the main window is visible or not such as by
...
** WinLAMP or anyother installed plugins that the current
**
...
/* (requires WinLAMP 5.666+)
** int has_video_support=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_HAS_VIDEO_SUPPORT);
...
** the no_video=1 winlamp.ini configuration option) and 1 if enabled (so will return
** 1 on all older WinLAMP clients where this API method is not available).
*/
...
// these messages are callbacks that you can grab by subclassing the winlamp window
...
** {
**   if(SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_ISPLAYING) == 1 &&
**      !SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETOUTPUTTIME))
**   {
**     char* file = (char*)SendMessageW(hwnd_winlamp,WM_WA_IPC,
**                  SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETLISTPOS),IPC_GETPLAYLISTFILE);
**     // only output if a valid file was found
**     if(file)
**     {
**       MessageBox(hwnd_winlamp,file,"starting",0);
...
/* (requires WinLAMP 2.9+)
** int newpos=SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)adjust_offset,IPC_ADJUST_FFWINDOWSMENUPOS);
** This will move where WinLAMP expects the freeform windows in the menubar windows main
...
/* (requires WinLAMP 5.0+)
** int dsize=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_ISDOUBLESIZE);
** You send this to WinLAMP to query if the double size mode is enabled or not.
...
/* (requires WinLAMP 2.9+)
** int newpos=SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)adjust_offset,IPC_ADJUST_FFOPTIONSMENUPOS);
** moves where winlamp expects the freeform preferences item in the menubar windows main
...
/* (requires WinLAMP 5.0+)
** int mode=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETTIMEDISPLAYMODE);
** This will return the status of the time display i.e. shows time elapsed or remaining.
** This returns 0 if WinLAMP is displaying time elapsed or 1 for the time remaining.
*/


#define IPC_SETVISWND 611
/* (requires WinLAMP 5.0+)
** int viswnd=(HWND)SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(HWND)viswnd,IPC_SETVISWND);
...
** See %SDK%\winlamp\wa5vis.txt for more info about visualisation integration in WinLAMP.
*/
...
/* (requires WinLAMP 5.0+)
** int viswnd=(HWND)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETVISWND);
...
/* (requires WinLAMP 5.0+)
** int visrunning=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_ISVISRUNNING);
...
/* (requires WinLAMP 5.0+)
** This is sent by WinLAMP back to itself so it can be trapped and adjusted as needed with
...
/* (requires WinLAMP 5.0+)
** int sovc=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETSTOPONVIDEOCLOSE);
...
/* (requires WinLAMP 5.0+)
** int sovc=SendMessageW(hwnd_winlamp,WM_WA_IPC,enabled,IPC_SETSTOPONVIDEOCLOSE);
...
/* (requires WinLAMP 5.0+)
** HWND prefs = (HWND)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETPREFSWND);
...
** SendMessageW(hwnd_winlamp,WM_COMMAND,MAKEWPARAM(WINLAMP_OPTIONS_PREFS,0),0);
** MessageBox(hwnd_winlamp,(IsWindow((HWND)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETPREFSWND))?"Valid":"Not Open"),0,MB_OK);
*/


#define IPC_SET_PE_WIDTHHEIGHT 620
/* (requires WinLAMP 5.0+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)&point,IPC_SET_PE_WIDTHHEIGHT);
** You pass a pointer to a POINT structure which holds the width and height and WinLAMP
...
/* (requires WinLAMP 5.0+)
** HINSTANCE hInst = (HINSTANCE)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETLANGUAGEPACKINSTANCE);
** This will return the HINSTANCE to the currently used language pack file for winlamp.exe
**
** (5.5+)
** If you pass 1 in wParam then you will have zero returned if a language pack is in use.
** if(!SendMessageW(hwnd_winlamp,WM_WA_IPC,1,IPC_GETLANGUAGEPACKINSTANCE)){
**   // winlamp is currently using a language pack
** }
**
** If you pass 2 in wParam then you will get the path to the language pack folder.
** wchar_t* lngpackfolder = (wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,2,IPC_GETLANGUAGEPACKINSTANCE);
**
** If you pass 3 in wParam then you will get the path to the currently extracted language pack.
** wchar_t* lngpack = (wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,3,IPC_GETLANGUAGEPACKINSTANCE);
**
** If you pass 4 in wParam then you will get the name of the currently used language pack.
** wchar_t* lngname = (char*)SendMessageW(hwnd_winlamp,WM_WA_IPC,4,IPC_GETLANGUAGEPACKINSTANCE);
*/
...
**
** wchar_t* ident_str = (wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,MAKEWPARAM(5,LANG_XXX),IPC_GETLANGUAGEPACKINSTANCE);
**
** e.g.
...
** the default winlamp.exe instance.
**
** HINSTANCE lang = (HINSTANCE)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETLANGUAGEPACKINSTANCE);
...
/* (requires WinLAMP 5.0+)
** char* outdll = (char*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETOUTPUTPLUGIN);
...
/* (requires WinLAMP 5.0+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,enabled,IPC_SETDRAWBORDERS);
** Set enabled to 1 to enable and 0 to disable drawing of the playlist editor and winlamp
...
/* (requires WinLAMP 5.0+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,enabled,IPC_DISABLESKINCURSORS);
...
/* (requires WinLAMP 5.0+)
** int val=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_IS_FULLSCREEN);
...
/* (requires WinLAMP 5.0+)
** A vis should send this message with 1/as param to notify winlamp that it has gone to or has come back from fullscreen mode
*/


#define IPC_SHOW_NOTIFICATION 632
/* (requires WinLAMP 5.0+ & Modern skin support - gen_ff.dll present)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_SHOW_NOTIFICATION);
...
/* (requires WinLAMP 5.0+)
** This is a notification message sent to the main WinLAMP window by itself whenever it
** needs to get information to be shown about the current skin in the 'Current skin
** information' box on the main Skins page in the WinLAMP preferences.
**
** When this notification is received and the current skin is one you are providing the
** support for then you return a valid buffer for WinLAMP to be able to read from with
...
/* (requires WinLAMP 5.03+)
** int val=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_MANUALPLADVANCE);
...
/* (requires WinLAMP 5.03+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,value,IPC_SET_MANUALPLADVANCE);
...
/* (requires WinLAMP 5.04+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_NEXT_PLITEM);
**
** Sent to WinLAMP's main window when an item has just finished playback or the next
...
** Return -1 for normal WinLAMP operation (default) or the new item number in
...
/* (requires WinLAMP 5.04+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_PREVIOUS_PLITEM);
**
** Sent to WinLAMP's main window when the previous button has been pressed and WinLAMP is
...
/* (requires WinLAMP 5.04+)
** int is_shaded=SendMessageW(hwnd_winlamp,WM_WA_IPC,wnd,IPC_IS_WNDSHADE);
...
/* (requires WinLAMP 5.04+ with ML)
** int rating=SendMessageW(hwnd_winlamp,WM_WA_IPC,rating,IPC_SETRATING);
...
** int cur_pos=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETLISTPOS);
**   SendMessageW(hwnd_winlamp,WM_WA_IPC,item_to_set,IPC_SETPLAYLISTPOS);
**   SendMessageW(hwnd_winlamp,WM_WA_IPC,rating_to_set,IPC_SETRATING);
**   SendMessageW(hwnd_winlamp,WM_WA_IPC,cur_pos,IPC_SETPLAYLISTPOS);
** }
**
** Note: WinLAMP 5.6+ allows you to save the rating directly into the file as long as the
...
/* (requires WinLAMP 5.04+ with ML)
** int rating=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETRATING);
...
** int cur_pos=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETLISTPOS), rating = 0;
**   SendMessageW(hwnd_winlamp,WM_WA_IPC,item_to_get,IPC_SETPLAYLISTPOS);
**   rating = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETRATING);
**   SendMessageW(hwnd_winlamp,WM_WA_IPC,cur_pos,IPC_SETPLAYLISTPOS);
**   return rating;
** }
*/
...
/* (requires WinLAMP 5.04+)
** int n = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETNUMAUDIOTRACKS);
...
/* (requires WinLAMP 5.04+)
** int n = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETNUMVIDEOTRACKS);
...
/* (requires WinLAMP 5.04+)
** int cur = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETAUDIOTRACK);
...
/* (requires WinLAMP 5.04+)
** int cur = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETVIDEOTRACK);
...
/* (requires WinLAMP 5.04+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,track,IPC_SETAUDIOTRACK);
...
/* (requires WinLAMP 5.04+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,track,IPC_SETVIDEOTRACK);
...
/* (requires WinLAMP 5.04+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_PUSH_DISABLE_EXIT);
...
** was required that needed to prevent exit otherwise you have to kill the WinLAMP process.
*/


#define IPC_POP_DISABLE_EXIT  648
/* (requires WinLAMP 5.04+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_POP_DISABLE_EXIT);
...
/* (requires WinLAMP 5.04+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_IS_EXIT_ENABLED);
** This will return 0 if the 'exit' option of WinLAMP's menu is disabled and 1 otherwise.
*/


#define IPC_IS_AOT 650
/* (requires WinLAMP 5.04+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_IS_AOT);
...
/* (requires WinLAMP 5.09+)
** int use_bin=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_USES_RECYCLEBIN);
...
**
** Note: if you use this on pre 5.09 installs of WinLAMP then it will return 1 which is
...
**
** if(SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_USES_RECYCLEBIN) &&
**    SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETVERSION)>=0x5009)
** {
...
/* (requires WinLAMP 5.36+)
** int show_state = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_INITIAL_SHOW_STATE);
** returns the processed value of nCmdShow when WinLAMP was started
** (see MSDN documentation the values passed to WinMain(..) for what this should be)
**
** e.g.
** if(SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_INITIAL_SHOW_STATE) == SW_SHOWMINIMIZED){
**   // we are starting minimised so process as needed (keep our window hidden)
** }
**
** Useful for seeing if winlamp was run minimised on startup so you can act accordingly.
** On pre-5.36 versions this will effectively return SW_NORMAL/SW_SHOWNORMAL due to the
** handling of unknown apis returning 1 from WinLAMP.
*/


#define IPC_GET_STOP_AFTER_CURRENT 658
/* (requires WinLAMP 5.58+)
** int stopping=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_STOP_AFTER_CURRENT);
...
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(const char*)colour_theme_name,IPC_FF_ONCOLORTHEMECHANGED);
**
...
/*
** int ismainwnd = (HWND)SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(HWND)test_wnd,IPC_FF_ISMAINWND);
**
...
** return a positive test result for a number of window handles within the current WinLAMP
...
** enumerate all windows related to WinLAMP at the time and allows you to process as
...
**
**   if(SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)hwnd,IPC_FF_ISMAINWND)){
...
/*
** HWND wa2embed = (HWND)SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(HWND)test_wnd,IPC_FF_GETCONTENTWND);
**
** This will return the WinLAMP 2 window that is embedded in the window's container
** i.e. if hwnd is the playlist editor windowshade hwnd then it will return the WinLAMP 2
...
** SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(const char*)hotkey_desc,IPC_FF_NOTIFYHOTKEY);
*/
...
/*
** General IPC messages in WinLAMP
**
...
/* (requires WinLAMP 5.0+)
** IDropTarget* IDrop = (IDropTarget*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GETDROPTARGET);
**
** You call this to retrieve a copy of the IDropTarget interface which WinLAMP created for
...
** same drag and drop support as WinLAMP normally provides the user. Check out MSDN or
...
/* (requires WinLAMP 5.0+)
** This is a notification message sent to the main WinLAMP window whenever the playlist is
...
** will slow down WinLAMP as playlist entries are modified (especially when you're adding
** in a large playlist).
**
...
/* (requires WinLAMP 5.0+)
** This is a notification message sent to the main WinLAMP window when playback begins for
...
/* (requires WinLAMP 5.66+)
** This is a notification message sent to the main WinLAMP window when a file is about to
...
/* (requires WinLAMP 5.0+)
** This is a notification message sent to the main WinLAMP window when a file's tag
...
/* (requires WinLAMP 5.0+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,allow,IPC_ALLOW_PLAYTRACKING);
...
** whilst playing that track so this can be sent to WinLAMP so it appears like
...
/* (requires WinLAMP 5.0+)
** This is a notification message sent to the main WinLAMP window asking if it's okay to
** close or not. Return zero to prevent WinLAMP from closing or return anything non-zero
** to allow WinLAMP to close.
**
...
** with regards to WinLAMP closing.
**
...
/* (requires WinLAMP 5.0+)
** SendMessageW(hwnd_winlamp,WM_WA_IPC,write_type,IPC_WRITECONFIG);
**
...
/* (requires WinLAMP 5.1+)
** int changed=SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)(char*)field,IPC_METADATA_CHANGED);
** a plugin can SendMessageW this to winlamp if internal metadata has changes.
...
** WinLAMP for it at the moment.
*/


#define IPC_SKIN_CHANGED 3018
/* (requires WinLAMP 5.1+)
** This is a notification message sent to the main WinLAMP window by itself whenever
...
** {
**   // do what you need to do to handle skin changes, e.g. call WADlg_init(hwnd_winlamp);
** }
*/
...
/* (requires WinLAMP 5.1+)
** WORD id = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_REGISTER_LOWORD_COMMAND);
...
** when the main resource file of WinLAMP is updated with extra data so assumptions cannot
...
typedef struct 
{
  ...
  DWORD id; // filled in by winlamp on return
} DispatchInfo;

#define IPC_GET_DISPATCH_OBJECT 3020  // gets winlamp main IDispatch * (for embedded webpages)
#define IPC_GET_UNIQUE_DISPATCH_ID 3021 // gives you a unique dispatch ID that won't conflict with anything in winlamp's IDispatch *
#define IPC_ADD_DISPATCH_OBJECT 3022 // add your own dispatch object into winlamp's.  This lets embedded webpages access your functions
// pass a pointer to DispatchInfo (see below).  WinLAMP makes a copy of all this data so you can safely delete it later

#define IPC_REMOVE_DISPATCH_OBJECT 3038 
/* (requires WinLAMP 5.56+)
...
/* (requires WinLAMP 5.11+)
** char* proxy_string=(char*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_PROXY_STRING);
...
/* (requires WinLAMP 5.11+)
** int reg_enabled=SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_USE_REGISTRY);
** This will return 0 if you should leave your grubby hands off the registry (i.e. for
** lockdown mode). This is useful if WinLAMP is run from a USB drive and you can't alter
** system settings, etc.
*/


#define IPC_GET_API_SERVICE 3025 
/* (requires WinLAMP 5.12+)
** api_service* api_service = (api_service*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_API_SERVICE);
** This api will return WinLAMP's api_service pointer (which is what WinLAMP3 used, heh).
** If this api is not supported in the WinLAMP version that is being used at the time then
** the returned value from this api will be 1 which is a good version check.
**
** As of 5.12 there is support for .w5s plugins which reside in %WinLAMPDir%\System and
...
/* (requires 5.2+)
** int pl_item = SendMessageW(hwnd_winlamp,WM_WA_IPC,start,IPC_PLAYLIST_GET_NEXT_SELECTED);
**
...
** while((sel = SendMessageW(hwnd_winlamp,WM_WA_IPC,sel,IPC_PLAYLIST_GET_NEXT_SELECTED))!=-1){
**   // show the playlist file entry of the selected item(s) if there were any
**   MessageBox(hwnd_winlamp,(char*)SendMessageW(hwnd_winlamp,WM_WA_IPC,sel,IPC_GETPLAYLISTFILE),0,0);
** }
*/
...
/* (requires 5.2+)
** int selcnt = SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_PLAYLIST_GET_SELECTED_COUNT);
...
/* (requires WinLAMP 5.?+)
** wchar_t* fn=(wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_PLAYING_FILENAME);
** This will return the currently playing filename string as held by WinLAMP.
*/
...
/* (requires WinLAMP 5.35+)
** int ret = SendMessageW(hwnd_winlamp,WM_WA_IPC,param,IPC_USE_UXTHEME_FUNC);
...
** for unsupported ipc calls on older WinLAMP versions)
*/
  #define IPC_ISWINTHEMEPRESENT 0
/* This will return 0 if uxtheme.dll is present
** int isthemethere = !SendMessageW(hwnd_winlamp,WM_WA_IPC,IPC_ISWINTHEMEPRESENT,IPC_USE_UXTHEME_FUNC);
*/
  #define IPC_ISAEROCOMPOSITIONACTIVE 1
/* This will return 0 if aero composition is active
** int isaero = !SendMessageW(hwnd_winlamp,WM_WA_IPC,IPC_ISAEROCOMPOSITIONACTIVE,IPC_USE_UXTHEME_FUNC);
*/


#define IPC_GET_PLAYING_TITLE 3034
/* (requires WinLAMP 5.5+)
** wchar_t* title=(wchar_t*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_PLAYING_TITLE);
** This will return the currently playing file title string as displayed in WinLAMP's window.
*/


#define IPC_CANPLAY 3035
/* (requires WinLAMP 5.5+)
** In_Module *in_mod=(In_Module*)SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)fn,IPC_CANPLAY);
...
typedef struct {
  ...
  // winlamp will fill these in if the call returns successfully:
...
** WinLAMP will copy the string, so don't worry about keeping it around
...
#define IPC_HANDLE_URI 3039 /* only for use in WM_COPYDATA, passes WinLAMP a filename that will get handled by svc_urihandler */
...
/* (requires WinLAMP 5.57+)
** This is a notification message sent to the main WinLAMP window by an output plugin
...
/* (requires WinLAMP 5.57+)
** HMODULE d3dx9 = (HMODULE)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_D3DX9);
**
** This will return the module handle of a d3dx9_*.dll based against the version
** internally required by WinLAMP and it's supporting plug-ins (version can change).
**
** HMODULE d3dx9 = (HMODULE)SendMessageW(hwnd_winlamp,WM_WA_IPC, 0, IPC_GET_D3DX9);
...
/* (requires WinLAMP 5.58+)
** IFileTypeRegistrar* registrar = (IFileTypeRegistrar*)SendMessageW(hwnd_winlamp,WM_WA_IPC,0,IPC_GET_FILEREGISTRAR_OBJECT);
**
** You call this to retrieve a copy of a IFileTypeRegistrar object which WinLAMP will
...
#define IPC_REGISTER_WINLAMP_IPCMESSAGE 65536 
/* (requires WinLAMP 5.0+)
** DWORD id = SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)name,IPC_REGISTER_WINLAMP_IPCMESSAGE);
** e.g. DWORD id = SendMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)&"enter_a_different_string",IPC_REGISTER_WINLAMP_IPCMESSAGE);
**
...
** common runtime api to be provided for the currently running instance of WinLAMP
** e.g.
**   PostMessageW(hwnd_winlamp,WM_WA_IPC,(WPARAM)my_param,registered_ipc);
** Have a look at wa_hotkeys.h for an example on how this api is used in practice for a
** custom WM_WA_IPC message.
**
**
** if(uMsg == WM_WA_IPC && lParam == id_from_register_winlamp_ipcmessage){
**   // do things
** }
*/
...
** WinLAMP misc commands.
** 
** To send these, use:
**
** SendMessageW(hwnd_winlamp, WM_COMMAND,command_name,0);
*/
...
#define WINLAMP_FILE_DIR                 40187 // pops up the load directory box
