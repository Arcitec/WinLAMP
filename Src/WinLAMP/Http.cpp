...
_redirect_goto:
	...
	// determine if proxy server used
	{
		...
		if (url_port != 80 && GetPrivateProfileIntW(L"WinLAMP",L"proxyonly80",0,INI_FILE))
			proxy="";
	}
...
static BOOL CALLBACK rf_DlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam)
{
	...
	switch (uMsg)
	{
		...
		case WM_INITDIALOG:
		{
			...
			if ( strstr( rf_url, "client.winlamp.com/update" ) )
				SetDlgItemTextA( hwndDlg, IDC_URL, getString( IDS_HTTP_WINLAMP_UPDATE_SITE, errorStr, 1024 ) );
			else
...
int httpRetrieveFileW(HWND hwnd, const char *url, const wchar_t *file, const wchar_t *dlgtitle)
{
...

	{
		...
		{
//			MessageBox(hwnd,getString(IDS_ONLYHTTP,NULL,0),"WinLAMP",MB_OK|MB_ICONSTOP);
			rf_in=0;
			return 1;
		}
	}
