...
static winlampVisModule *vis_mod;
...
static winlampVisModule *GetVis()
{
	winlampVisModule *ret = 0;
...
static void ReleaseVis(winlampVisModule *vis)
{
	if (vis)
	{
		EnterCriticalSection(&cs);
...
void vis_stop()
{
	...
	winlampVisModule *thisVis = vis_mod;
	LeaveCriticalSection(&cs);
	ReleaseVis(thisVis); 
}
...
static winlampVisModule *CreateVis(HINSTANCE visLib)
{
	EnterCriticalSection(&cs);
	winlampVisModule *ret = 0;
	ret = GetVis();
	...
	winlampVisGetHeaderType pr;
	pr = (winlampVisGetHeaderType) GetProcAddress(visLib,"winlampVisGetHeader");
	if (!pr)
	{
		LeaveCriticalSection(&cs);
		return 0;
