...
static HWND find_otherwinlamp_fast()
{
...
wchar_t *ParseParameters(wchar_t *lpszCmdParam, int *bAdd, int *bBookmark, int *bHandle, int *nCmdShow, int *bCommand, int *bCmdParam, int *bAllowCompat)
{
	for (;;)
	{
		...
		{
			// winlamp.exe /ADDPLAYLIST playlist.m3u "Playlist Name" {GUID}
		...
		{
			// winlamp.exe /CREATEPLAYLIST "Playlist Name" {GUID}
		...
		{
			// winlamp.exe /APPENDPLAYLIST {GUID} filename.mp3
			lpszCmdParam = SkipXW(lpszCmdParam, 15);
			setup_config();
			Wasabi_Load();
			w5s_init();
				 ...
				 IsCommand(lpszCmdParam, L"/CLOSE", 6))
		{
			...
			HWND hwnd_other_winlamp = find_otherwinlamp_fast();
			if (IsWindow(hwnd_other_winlamp))
				PostMessageW(hwnd_other_winlamp, WM_CLOSE, 0, 0);
			ExitProcess(0);
		}
		else if (IsCommand(lpszCmdParam, L"/KILL", 5))
		{
			HWND hwnd_other_winlamp;
			DWORD other_winlamp_procId = 0;
			lpszCmdParam = SkipXW(lpszCmdParam, 5);
			hwnd_other_winlamp = find_otherwinlamp_fast();
			if (hwnd_other_winlamp)
			{
				PostMessageW(hwnd_other_winlamp, WM_CLOSE, 0, 0);

				GetWindowThreadProcessId(hwnd_other_winlamp, &other_winlamp_procId); // get the process ID
				if (other_winlamp_procId) // if we didn't get one, it probably already handled the WM_CLOSE message ...
				{
					HANDLE other_winlamp_process = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_TERMINATE | SYNCHRONIZE, FALSE, other_winlamp_procId);
					if (other_winlamp_process) // if we got a process handle (it might have quit already in the meantime ...)
					{
						if (WaitForSingleObject(other_winlamp_process, 3000) == WAIT_TIMEOUT) // wait 5 seconds for it to close
						{
							DWORD exitCode = 0;
							TerminateProcess(other_winlamp_process, exitCode); // terminate if we timed out
							WaitForSingleObject(other_winlamp_process, 1000); // wait some more because TerminateProcess() returns immediately
						}
						CloseHandle(other_winlamp_process); // release our reference to the handle
					}
				}
			}
			RemoveRegistrar();
		...
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 6);
			*bCommand = MAKEWPARAM(WINLAMP_BUTTON1, 0);
		}
		else if (IsCommand(lpszCmdParam, L"/PLAY", 5))
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 6);
			*bCommand = MAKEWPARAM(WINLAMP_BUTTON2, 0);
		}
		else if (IsCommand(lpszCmdParam, L"/PAUSE", 6))
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 7);
			*bCommand = MAKEWPARAM(WINLAMP_BUTTON3, 0);
		}
		else if (IsCommand(lpszCmdParam, L"/STOP", 5))
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 6);
			*bCommand = MAKEWPARAM(WINLAMP_BUTTON4, 0);
		}
		else if (IsCommand(lpszCmdParam, L"/STOPFADE", 9))
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 10);
			*bCommand = MAKEWPARAM(WINLAMP_BUTTON4_SHIFT, 0);
		}
		else if (IsCommand(lpszCmdParam, L"/STOPAFTER", 10))
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 11);
			*bCommand = MAKEWPARAM(WINLAMP_BUTTON4_CTRL, 0);
		}
		else if (IsCommand(lpszCmdParam, L"/NEXT", 5))
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 6);
			*bCommand = MAKEWPARAM(WINLAMP_BUTTON5, 0);
		}
		...
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 4);
			*bCommand = MAKEWPARAM(WINLAMP_BUTTON5_SHIFT, 0);
		}
		else if (IsCommand(lpszCmdParam, L"/REV", 4))
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 4);
			*bCommand = MAKEWPARAM(WINLAMP_BUTTON1_SHIFT, 0);
		}
		...
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 7);
			*bCommand = MAKEWPARAM(WINLAMP_FILE_SHUFFLE, 0);
		}
		...
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 7);
			*bCommand = MAKEWPARAM(WINLAMP_FILE_REPEAT, 0);
		}
		...
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 6);
			*bCommand = MAKEWPARAM(WINLAMP_VOLUMEUP, 0);
		}
		else if (IsCommand(lpszCmdParam, L"/VOLDOWN", 8))
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 8);
			*bCommand = MAKEWPARAM(WINLAMP_VOLUMEDOWN, 0);
		}
		...
		{
			lpszCmdParam = SkipXW(lpszCmdParam, 8);
			*bCommand = MAKEWPARAM(WINLAMP_JUMPFILE, 0);
		}
	...
	if ((!mode && (!_wcsicmp(buf, L"wsz") || !_wcsicmp(buf, L"wal"))) || (mode && !_wcsicmp(buf, L"wlz")))
	{
		...
		if (hwnd_other)
		{
			...
			else
			{
				...
				PostMessageW(hwnd_other,WM_USER,0,IPC_RESTARTWINLAMP);
			}
			*exit=1;
		}
		else
