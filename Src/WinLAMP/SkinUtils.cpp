...
const char *GetFontName()
{
	...
	if (!Skin_PLFont[0])
	{
		static char lang_font[128]; // benski> this is thread-safe because the language pack won't change (changing lang packs requires winlamp restart)
...
const wchar_t *GetFontNameW()
{
	...
	if (!Skin_PLFontW[0])
	{
		static wchar_t lang_fontW[128]; // benski> this is thread-safe because the language pack won't change (changing lang packs requires winlamp restart)
