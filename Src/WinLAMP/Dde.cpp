...
//CreateShortCut(hwnd,"D:\\WINNT\\Profiles\\Administrator\\Application Data\\Microsoft\\Internet Explorer\\Quick Launch\\WinLAMP.lnk","C:\\winlamp\\winlamp.exe");
//CreateShortCut(hwnd,"D:\\WINNT\\Profiles\\Administrator\\Desktop\\WinLAMP.lnk","C:\\winlamp\\winlamp.exe");
...
void dde_addquicklaunch(HWND hwnd)
{
	...
	if (lstrlen(name))
	{
		char exe[MAX_PATH];
		GetModuleFileName(NULL,exe,sizeof(exe));
		lstrcat(name,"\\Microsoft\\Internet Explorer\\Quick Launch\\WINLAMP.LNK");
		CreateShortCut(hwnd,name,exe,NULL,0);
	}
}
	...
	if (RegOpenKey(HKEY_CURRENT_USER,"Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Shell Folders",&mp3Key) == ERROR_SUCCESS) 
	{
      ...
      char exe[MAX_PATH], shortcut[MAX_PATH], target[MAX_PATH];
	    GetModuleFileName(NULL,exe,sizeof(exe));
			...
			// create WinLAMP folder
			PathAppend(startMenu, "WinLAMP");
      CreateDirectory(startMenu, NULL);

			// winlamp.exe
			PathCombine(shortcut, startMenu, "WinLAMP.lnk");
	    ...
	    CreateShortCut(hwnd, startMenu, target, NULL, 0);
			...
			// uninstwa.exe
			PathCombine(shortcut, startMenu, "Uninstall WinLAMP.lnk");
