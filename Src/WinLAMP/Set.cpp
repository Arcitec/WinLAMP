...

		{
			...
			for (int x=0; x < sizeof(rs)/sizeof(rs[0]); x++) 
			{
				...
				if(!x) ti.lpszText = getStringW(IDS_PREAMP,NULL,0);
					...
					wchar_t HZStr[16] = {0};
					getStringW((x<5+!(config_eq_frequencies==EQ_FREQUENCIES_WINLAMP)?IDS_EQ_HZ:IDS_EQ_KHZ),HZStr,16);
					StringCchPrintfW(buf,80,L"%s%s",
									 ((config_eq_frequencies==EQ_FREQUENCIES_WINLAMP)?bands[x]:bandsISO[x]),
									 (!x?L"":HZStr));
					ti.lpszText = buf;
				}
...
void set_aot(int dodockingstuff) 
{
	...
	CheckMenuItem(main_menu,WINLAMP_OPTIONS_AOT,config_aot?MF_CHECKED:MF_UNCHECKED);
	CheckMenuItem(main_menu,WINLAMP_OPTIONS_DSIZE,config_dsize?MF_CHECKED:MF_UNCHECKED);
