...
static void get_objlist(char l[256])
{
	...
	GetPrivateProfileStringA("winlamp","browserbrand","",browserbrand+1,sizeof(browserbrand)-1,INI_FILEA);
	if (browserbrand[1])
		browserbrand[0]='.';
...
BOOL SendMetrics(const char *data, HWND hwndParent)
{
	...
	char *url = urlbuf;
	int is_upgrade=!GetPrivateProfileIntW(L"WinLAMPReg",L"IsFirstInst",0,INI_FILE);

	if (!prd) return 0;

	// fucko: set is_upgrade
	StringCchCopyExA(url,urlsize,"http://client.winlamp.com/update/do_im.php?", &url, &urlsize, 0);
...
//				prd->agegroup,
	...
	if (result) WritePrivateProfileStringW(L"WinLAMPReg", L"IsFirstInst", L"0", INI_FILE);
	return result;
}