...
void ll_update(HWND hwndDlg)
{
	...
	lstrcat(TEMPFILE2,"0");
	{
		...
		wsprintf(url,"http://client.winlamp.com/update/updatelinks.jhtml?i=%c&v=%s&r=%c",
			c,
			app_version,r);
      ...
      char bnav[1024]="";
		  ...
		  lstrcat(d_bloc," ");
			if (fp) 
			{
				while (!feof(fp))
				{
					...
					if (!strcmp(buf,"WinLAMP Links File v1.0\n")) st|=1;
        ...
        success=1;
			...
			{
				char str[256]="";
				char *s="http://client.winlamp.com/update/mb.html";
				GetPrivateProfileString("WinLAMP","MBDefLoc",s,str,sizeof(str),INI_FILE);
        ...
        else if (isInetAvailable()) 
        {
          ...
          else mbctl_navigate("http://client.winlamp.com/browser/","WinLAMP Minibrowser");
        }
        else mbctl_navigate(MBFILE,"WinLAMP Minibrowser");
			}
		}
