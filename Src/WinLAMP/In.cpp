...
static void LoadInputPlugin(const wchar_t* filename)
{
	...
	In_Module* (*pr)();
	pr = (In_Module * (__cdecl*)(void)) GetProcAddress(hLib, "winlampGetInModule2");
	if ( pr == NULL )
		return;
	...
	if ( !g_has_video_plugin )
	{
		...
		gefiW = (int(__cdecl*)(const wchar_t*, const char*, wchar_t*, int))GetProcAddress(hLib, "winlampGetExtendedFileInfoW");
		if ( gefiW )
		{
			wchar_t dest[16] = { 0 };
		...
		gefi = (int(__cdecl*)(const char*, const char*, char*, int))GetProcAddress(hLib, "winlampGetExtendedFileInfo");
		if ( gefi )
		{
			char dest[16] = { 0 };
			gefi("", "type", dest, 16);
...
int in_infobox(HWND hwnd, const wchar_t* fn)
{
	...
	UseUnifiedFileInfoDlg uufid = (UseUnifiedFileInfoDlg)GetProcAddress(mod->hDllInstance, "winlampUseUnifiedFileInfoDlg");
