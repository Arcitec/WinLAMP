 ...
 ** Filename:  main.cpp
 ** Project: WinLAMP
 ** Description: WinLAMP initialization code
...
const char app_name[] = "WinLAMP", app_version[] = APP_VERSION, app_version_string[] = APP_VERSION_STRING; // application name and version strings
int g_fullstop;
...
HWND find_otherwinlamp( wchar_t * );
...
void BuildAppName()
{
	StringCchCopyW( szAppName, 64, L"WinLAMP v1.x" );
...
int CreateMainWindow()
{
	if ( !IsWindow( hMainWindow ) )
	{
		...
		if ( !CreateWindowExW( WS_EX_ACCEPTFILES, szAppName, L"WinLAMP", WS_OVERLAPPED | WS_SYSMENU | WS_MINIMIZEBOX | WS_BORDER | WS_CAPTION,
...
BOOL parseMessageCommands( HWND hwnd_other, int bCommand, int bCmdParam )
{
	if ( LOWORD( bCommand ) ) // attempt to send the action to it
	{
			...
			else if ( LOWORD( bCommand ) == IPC_ISPLAYING )
			{
				...
				switch ( command )
				{
					case 1:		// playing so need to pause
						command = WINLAMP_BUTTON3;
						break;
					default:	// stopped so start playing
						command = WINLAMP_BUTTON2;
						break;
				}
		...
		else
		{
			...
			if ( LOWORD( bCommand ) == WINLAMP_JUMPFILE )
			{
...
static int PassToOtherWinLAMP( wchar_t *lpszCmdParam, HWND hwnd_other, int bAdd, int bBookmark, int bHandle, int bCommand, int bCmdParam )
{
	// if we have command line params, pass to other winlamp window
	if ( lpszCmdParam && *lpszCmdParam )
	{
		...
		// check if we're using a language pack with the already open winlamp process
		// and if so then we're going to use the winlamp.lng from it on the messagebox
		// only if we had a success and the other winlamp returned the correct value
				...
				do
				{
					...
					if ( templng )
					{
						...
						if ( LoadStringW( templng, LANG_DLL_GUID_STRING_ID, s, 39 ) )
						{
							...
							{
								existingWLZ = Lang_FakeWinLAMPLangHInst( templng );
							}
							else
								FreeLibrary( templng );
						}
		...
		if ( skinExit )
		{
			...
			if ( existingWLZ )
			{
				Lang_FakeWinLAMPLangHInst( existingWLZ );
				FreeLibrary( templng );
			}
			return TRUE;
		}
		...
		hSem = CreateSemaphoreA( 0, 0, 65535, "WinLAMPExplorerHack1" );
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInst, LPSTR unused /*lpszCmdParam*/, int nCmdShow )
{
	...
	if ( ( now - compile ) >= 2678400 )
	{
		...
		MSGBOXPARAMSW msgbx = {
			...
			L"This beta version of WinLAMP is now over 4 weeks old.\n\n"
			L"Please update to the latest WinLAMP version available.",
			L"WinLAMP Beta Expired",
			MB_USERICON,
			MAKEINTRESOURCEW( 102 ),
			0, 0, 0
		};
		MessageBoxIndirectW( &msgbx );
		ShellExecuteW( NULL, L"open", L"http://www.winlamp.com/media-player", NULL, NULL, SW_SHOWNORMAL );
		return 0;
	}
#endif
#endif
	...
	// load supporting dlls from the WinLAMP\Shared directory
	...
	// don't incorrectly prompt about entering it from using IPC_RESTARTSAFEWINLAMP.
	...
	if ( !bAllowCompat && read_compatmode() )
	{
		MSGBOXPARAMSW msgbx = {
			...
			0,
			GetModuleHandle( NULL ),
			L"WinLAMP appears to have been started with Windows program compatibility mode enabled.\n\n"
			L"This is not a recommended way to run WinLAMP as it can often cause problems with how WinLAMP works e.g. causing it to randomly crash."
			L"\n\n\nAre you sure you want to continue to run WinLAMP like this?\n\n\n"
			L"If you choose 'No', you can disable this by right-clicking winlamp.exe, choosing 'Properties' and selecting the 'Compatibility' tab, "
			L"followed by unchecking 'Run this program in compatibility mode for:' and run WinLAMP again.",
			L"WinLAMP",
	...
	if ( 0 == ( 128 & is_install ) )
	{
		HWND hwnd_other = find_otherwinlamp( lpszCmdParam );
		if ( IsWindow( hwnd_other ) )
		{
			// unable to start safe mode so inform the user
			ShowSafeModeMessage( 1 );
			int x = PassToOtherWinLAMP( lpszCmdParam, hwnd_other, bAdd, bBookmark, bHandle, bCommand, bCmdParam );
			CoUninitialize();
			return x;
		}
	}
	...
	if ( hm )
	{
		...
		*(FARPROC *)&StartHandler = GetProcAddress( hm, "StartHandler" );
		if ( StartHandler )
		{
			...
			{
				winlampGeneralPurposePluginGetter pr = (winlampGeneralPurposePluginGetter)GetProcAddress( hm, "winlampGetGeneralPurposePlugin" );
				if ( pr )
				{
					winlampGeneralPurposePlugin *plugin = pr();
...
DWORD CALLBACK MainThread( LPVOID param )
{
	...
	if ( ( 128 & is_install ) || ( !g_noreg && GetPrivateProfileIntW( L"WinLAMPReg", L"NeedReg", 1, INI_FILE ) ) )
	{
	...
	if ( !g_safeMode )
	{
		language_pack_instance = Lang_InitLangSupport( hMainInstance, WinLAMPLangGUID );
	...
	songChangeBroadcastMessage = RegisterWindowMessageW( L"WinLAMPSongChange" );
	...
	{
		RemoveMenu( main_menu, WINLAMP_OPTIONS_VIDEO, MF_BYCOMMAND );
		RemoveMenu( GetSubMenu( v5_top_menu, 3 ), WINLAMP_OPTIONS_VIDEO, MF_BYCOMMAND );
...

	{
		...
		if ( v != 0 )
		{
			...
			_w_i( "show_prefs", 0 );
			PostMessageW( hMainWindow, WM_COMMAND, WINLAMP_OPTIONS_PREFS, 0 );
	...
	{
		PlayList_getcurrent( FileName, FileTitle, FileTitleNum );
		SendMessageW( hMainWindow, WM_COMMAND, WINLAMP_BUTTON2, 0 );
	...
	WPARAM exitParam = WinLAMPMessageLoop();
	JSAPI1_Uninitialize();
	...
	switch ( uMsg )
	{
		case WM_INITMENUPOPUP:
		{
			...
			if ( hMenu == main_menu )
			{
				...
				l_menu_info.dwTypeData = getStringW( IDS_WINLAMP_MENUITEM, NULL, 0 );
				l_menu_info.cch = (UINT)wcslen( l_menu_info.dwTypeData );
				SetMenuItemInfoW( main_menu, 0, TRUE, &l_menu_info );
				EnableMenuItem( main_menu, WINLAMP_FILE_QUIT, MF_BYCOMMAND | ( g_exit_disabled ? MF_GRAYED : MF_ENABLED ) );
			}
					...
					do
					{
							 ...
							 !_wcsicmp( extensionW( d.cFileName ), L"wlz" ) )
						{
							...
							if ( d.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY )
							{
								...
								PathCombineW( check, check, L"winlamp.lng" );
		...
		case WM_CONTEXTMENU:
			SendMessageW( hwnd, WM_COMMAND, WINLAMP_MAINMENU, 0 );
			return 0;
		...
		case WM_MOUSEWHEEL:
		{
			...
			if ( 0 != dLines )
			{
				...
				{
					if ( dLines >= 0 )
						dLines = WINLAMP_FFWD5S;
					else
						dLines = WINLAMP_REW5S;
				}
				else
				{
					if ( dLines >= 0 )
						dLines = WINLAMP_VOLUMEUP;
					else
						dLines = WINLAMP_VOLUMEDOWN;
				}
		...
		{
			//MessageBoxA(NULL, "winlamp/live", "winlamp/live", MB_OK);
			OnThumbnailPreview();
		}
		break;
#endif
...
HWND find_otherwinlamp( wchar_t *cmdline )
{
	int y = 0;
...
again:
	...
	if ( g_hEventRunning && GetLastError() == ERROR_ALREADY_EXISTS )
	{
		...
		if ( !bNoHwndOther && ( !config_minst || *cmdline ) ) for ( x = 0; x < 40; x++ )
		{
			...
			if ( failed ) return NULL; // no valid winlamp windows, but one that fucked up
