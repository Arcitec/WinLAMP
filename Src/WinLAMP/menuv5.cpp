...
int V5_Options_Menu(HWND hwnd, int x, int y, int width, int height)
{
	...
	CheckMenuItem(options_menu, WINLAMP_OPTIONS_DSIZE, config_dsize ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(options_menu, WINLAMP_OPTIONS_AOT, config_aot ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(options_menu, WINLAMP_OPTIONS_ELAPSED, config_timeleftmode ? MF_UNCHECKED : MF_CHECKED);
	CheckMenuItem(options_menu, WINLAMP_OPTIONS_REMAINING, config_timeleftmode ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(options_menu, WINLAMP_OPTIONS_PREFS, IsWindow(prefs_hwnd) ? MF_CHECKED : MF_UNCHECKED);
...
int V5_Windows_Menu(HWND hwnd, int x, int y, int width, int height)
{
	...
	int flag = TPM_LEFTALIGN;
	CheckMenuItem(windows_menu, WINLAMP_OPTIONS_PLEDIT, config_pe_open ? MF_CHECKED : MF_UNCHECKED);
	CheckMenuItem(windows_menu, WINLAMP_VISPLUGIN, vis_running() ? MF_CHECKED : MF_UNCHECKED);
	if (g_has_video_plugin) CheckMenuItem(windows_menu, WINLAMP_OPTIONS_VIDEO, config_video_open ? MF_CHECKED : MF_UNCHECKED);
...
int Help_Menu(HWND hwnd, int x, int y, int width, int height)
{
	...
	i.fMask = MIIM_TYPE;
	i.fType = MFT_STRING;
	i.dwTypeData = getStringW(IDS_WINLAMP_MENUITEM, NULL, 0);
	i.cch = (UINT)wcslen(i.dwTypeData);
	SetMenuItemInfoW(help_menu, WINLAMP_HELP_ABOUT, FALSE, &i);
