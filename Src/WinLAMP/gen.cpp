...
std::vector<winlampGeneralPurposePlugin*> gen_plugins;
...
void LoadGenPlugin(const wchar_t* filename)
{
	...
	winlampGeneralPurposePlugin* (*pr)();
	pr = (winlampGeneralPurposePlugin * (__cdecl*)(void)) GetProcAddress(hLib, "winlampGetGeneralPurposePlugin");
	if ( pr == NULL )
	{
	...
	if ( plugin && (plugin->version == GPPHDR_VER || plugin->version == GPPHDR_VER_U) )
	{
		...
		if ( g_safeMode )
		{
			...
			else
			{
				...
				// otherwise they'll just cause WinLAMP to keep crashing!
