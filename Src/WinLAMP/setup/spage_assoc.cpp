...
static BOOL IsAgentScheduled(void)
{
	...
	if (ERROR_SUCCESS != RegOpenKeyW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", &hKey))
		return FALSE;
	if (ERROR_SUCCESS != RegQueryValueExW(hKey, L"WinLAMPAgent", NULL, NULL, (BYTE*)szAgent, &cb))
		szAgent[0] = 0x00;
	RegCloseKey(hKey);
...
static BOOL IsAgentExist(void)
{
	...
	PathRemoveFileSpecW(szAgent);
	if (NULL == PathCombineW(szAgent, szAgent, L"winlampa.exe"))
		return FALSE;
...
static void RegisterProtocols()
{
	...
	wchar_t winlampexe[MAX_PATH] = {0};
	GetModuleFileNameW(hMainInstance, winlampexe, MAX_PATH);
		...
		for(size_t i =0;;i++)
		{
			...
			if (NULL != handler)
			{
				...
				for (size_t k = 0; ;k++)
				{
					...
					if (0 == handler->RegisterProtocol(szName, winlampexe))
					{
...
{
	wchar_t szVer[512] = {0};
	if (0 == GetPrivateProfileIntW(L"WinLAMPReg", L"IsFirstInst", 1, INI_FILE)) return FALSE;

	GetPrivateProfileStringW(L"WinLAMP", L"config_extlist", L"", szVer, 512, INI_FILE);
	if (*szVer) return FALSE;

	GetPrivateProfileStringW(L"WinLAMPReg", L"WAVer", L"", szVer, 512, INI_FILE);
...
static BOOL GetAuxExtensionName(LPCWSTR pszExt, LPWSTR pszDest, INT cchDest)
{
	...
	if (CSTR_EQUAL == CompareStringW(lcid, NORM_IGNORECASE, pszExt, -1, L"wal", -1))
		result = (S_OK == StringCchCopyW(pszDest, cchDest, getStringW(IDS_WINLAMP_SKIN_MODERN, NULL, 0)));
	else if (CSTR_EQUAL == CompareStringW(lcid, NORM_IGNORECASE, pszExt, -1, L"wsz", -1))
		result = (S_OK == StringCchCopyW(pszDest, cchDest, getStringW(IDS_WINLAMP_SKIN_CLASSIC, NULL, 0)));
	else if (CSTR_EQUAL == CompareStringW(lcid, NORM_IGNORECASE, pszExt, -1, L"wlz", -1))
		result = (S_OK == StringCchCopyW(pszDest, cchDest, getStringW(IDS_WINLAMP_LANG_PACK, NULL, 0)));
	return result;
}
