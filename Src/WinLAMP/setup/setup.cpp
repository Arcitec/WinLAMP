...
WASetup::WASetup(void) 
	: ref((size_t)1), hwnd(NULL), hwndActive(NULL), nPageActive((size_t)-1), pui(NULL), hWinLAMP(NULL)
{	
}
...
HRESULT WASetup::Start(HWND hwndWinLAMP)
{
	INT_PTR r(IDOK);

	SetRectEmpty(&rcUI);

	hWinLAMP = hwndWinLAMP;
...
HRESULT WASetup::Save(HWND hwndStatus)
{
	...
	WritePrivateProfileStringW(L"WinLAMPReg", L"WAVer", AutoWide(APP_VERSION), INI_FILE);
	return hr;
}
...
HRESULT WASetup::GetWinLAMPWnd(HWND *phwndWinLAMP)
{
	if (NULL == phwndWinLAMP)
		return E_INVALIDARG;
	*phwndWinLAMP = hWinLAMP;
	return (NULL == hWinLAMP) ? E_UNEXPECTED : S_OK;
}
...
CB(API_SETUP_GETWINLAMPWND, GetWinLAMPWnd)
END_DISPATCH
#undef CBCLASS