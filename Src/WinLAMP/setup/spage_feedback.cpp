...
HRESULT setup_page_feedback::Save(HWND hwndText)
{
	...
	{
		s = GetPrivateProfileIntW(L"WinLAMPReg", L"RegDataLen", 0, INI_FILE);
		if (s > 0) GetPrivateProfileStructA("WinLAMPReg", "RegData2", data, s, INI_FILEA);
	...
	WritePrivateProfileStringA("WinLAMPReg", "RegDataLen", (s) ? str : NULL, INI_FILEA);
	WritePrivateProfileStructA("WinLAMPReg", "RegData2", (s) ? data : NULL, s, INI_FILEA);
...
HRESULT setup_page_feedback::Revert(void)
{
	...
	int s = GetPrivateProfileIntW(L"WinLAMPReg", L"RegDataLen", 0, INI_FILE);
	if (s > 0) GetPrivateProfileStructA("WinLAMPReg", "RegData2", data, s, INI_FILEA);
