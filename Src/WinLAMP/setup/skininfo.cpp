...
static BOOL ReadSkinInfo(LPCWSTR pszSkinXML, SKININFO *psi, LPWSTR pszImage, INT cchImage)
{
	...
	if (parserFactory)
	{
		...
		if (parser)
		{
			...
			parser->xmlreader_registerCallback(L"WinLAMPAbstractionLayer", &cb);
			parser->xmlreader_registerCallback(L"WinLAMPAbstractionLayer\fSkinInfo\f*", &cb);
...
void SkinXMLCallback::xmlReaderOnStartElementCallback(const wchar_t *xmlpath, const wchar_t *xmltag, ifc_xmlreaderparams *params)
{
	...
	if (CSTR_EQUAL == CompareStringW(lcid, NORM_IGNORECASE, L"WinLAMPAbstractionLayer", -1, xmltag, -1) ||
...
{	
												...
												pszSkinPath, -1, CLASSIC_NAME(), -1))
	{
		...
		if (SIF_COMMENT & psi->fMask) StringCchCopyW(psi->pszComment, psi->cchComment, L"WinLAMP base skin v5.5");
