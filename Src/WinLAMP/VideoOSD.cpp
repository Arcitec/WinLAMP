		...
		if (dragging == 1 || (x >= ctrlrect[i].left && x <= ctrlrect[i].right && y >= ctrlrect[i].top && y <= ctrlrect[i].bottom))
		{
			...
			switch (i)
			{
				...
				case CTRL_PAUSE:
					if (dragging == 0)
					{
						PostMessageW(hMainWindow, WM_COMMAND, WINLAMP_BUTTON3, 0);
					}
					return ;
				case CTRL_PLAY:
					if (dragging == 0)
					{
						PostMessageW(hMainWindow, WM_COMMAND, WINLAMP_BUTTON2, 0);
					}
					return ;
				case CTRL_STOP:
					if (dragging == 0)
				...
				case CTRL_FFWD:
					...
					{
						if (i == CTRL_REW)
							PostMessageW(hMainWindow, WM_COMMAND, WINLAMP_BUTTON1, 0);
						else
							PostMessageW(hMainWindow, WM_COMMAND, WINLAMP_BUTTON5, 0);
					}
					return ;
				default:
					if (dragging < 1)
