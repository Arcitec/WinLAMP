...
INT_PTR CALLBACK SetupProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam) 
{
	...
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			if (GetPrivateProfileIntW(L"WinLAMP",L"proxyonly80",0,INI_FILE)) CheckDlgButton(hwndDlg,IDC_PROXY_ONLY_PORT_80,BST_CHECKED);
			...
			if(!icon_tmp[0]) StringCchPrintfW(icon_tmp,MAX_PATH,L"%s\\winlamp.ico",CONFIGDIR);
		...
		case WM_COMMAND:
			switch (LOWORD(wParam))
			{
				...
				case IDC_PROXY_ONLY_PORT_80:
					...
					WritePrivateProfileStringW(L"WinLAMP",L"proxyonly80",config_proxy80?L"1":L"0",INI_FILE);
					return 0;
