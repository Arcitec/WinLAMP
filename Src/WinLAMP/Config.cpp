...
int GetRegistrar(IFileTypeRegistrar **registrar, BOOL use_fallback)
{
	...
	if (!masterRegistrar || usingBuiltIn)
	{
		...
		// without elevating WinLAMP itself - see http://msdn2.microsoft.com/en-us/ms679687.aspx
...
void config_read(int i)
{
	...
	//	regmimetype(L"interface/x-winlamp-skin", programname, L".wsz", 0); // hack cause wa3 is gheeeeeey
...
int config_isregistered(wchar_t *ext)
{
	...
	if (IsWin8()) // Windows 8
	{
		...
		if (SUCCEEDED(hr) && pAAR)
		{
			...
			{
				if(!lstrcmpiW(WINLAMP_PLAYLISTW, app)){ rval = 1; }
			}
			else if (!_wcsicmp(ext, L"wsz") || !_wcsicmp(ext, L"wal") || !_wcsicmp(ext, L"wpz"))
			{
				if(!lstrcmpiW(WINLAMP_SKINZIPW, app)){ rval = 1; }
			}
			else if (!_wcsicmp(ext, L"wlz"))
			{
				if(!lstrcmpiW(WINLAMP_LANGZIPW, app)){ rval = 1; }
			}
			else
			{
				wchar_t str[64] = {0};
				StringCchPrintfW(str, 64, L"%s%hs", WINLAMP_FILEW, b);
	...
	else
	{
		...
		{
			if (vt != REG_SZ || (wcsncmp(b, WINLAMP_FILEW, wcslen(WINLAMP_FILEW)) &&
								 wcscmp(b, WINLAMP_PLAYLISTW) &&
								 wcscmp(b, WINLAMP_SKINZIPW) &&
								 wcscmp(b, WINLAMP_LANGZIPW))) rval = 0;
			else rval = 1;
		}
		else rval = 0;
		RegCloseKey(key);
...
{
	const wchar_t *which_str = WINLAMP_FILEW;
	...
	if (GetRegistrar(&registrar, true) == 0 && registrar)
	{
		...
		if (IsPlaylistExtension(ext))
			which_str = WINLAMP_PLAYLISTW;
		else if (!_wcsicmp(ext, L"wsz") || !_wcsicmp(ext, L"wal") || !_wcsicmp(ext, L"wpz"))
			which_str = WINLAMP_SKINZIPW;
		else if (!_wcsicmp(ext, L"wlz"))
			which_str = WINLAMP_LANGZIPW;
		else
		{
			StringCchPrintfW(family_str, 256, L"%s.%s", WINLAMP_FILEW, ext);
		...
		if (reg && !_wcsicmp(ext, L"wal"))
		{
			wchar_t programname[MAX_PATH] = {0};
			GetModuleFileNameW(hMainInstance, programname, MAX_PATH);
			regmimetype(L"interface/x-winlamp3-skin", programname, L".wal", 0);
		}
...
void init_config()
{
	...
	SetEnvironmentVariableW(L"WINLAMP_PROGRAM_DIR", PROGDIR);
	wchar_t winlamp_root[MAX_PATH] = {0};
	StringCchCopyW(winlamp_root, MAX_PATH, PROGDIR);
	PathStripToRootW(winlamp_root);
	SetEnvironmentVariableW(L"WINLAMP_ROOT_DIR", winlamp_root);
}
...
void setup_config(void)
{
	...
	// basic config files
	PathCombineW(OLD_M3U_FILE, M3UDIR, L"WinLAMP.m3u");
	PathCombineW(M3U_FILE, M3UDIR, L"WinLAMP.m3u8");
	PathCombineW(BOOKMARKFILE, CONFIGDIR, L"WinLAMP.bm");
	PathCombineW(BOOKMARKFILE8, CONFIGDIR, L"WinLAMP.bm8");
	...
	PathCombineW(EQDIR1, CONFIGDIR, L"WinLAMP.q1");
	PathCombineW(EQDIR2, CONFIGDIR, L"WinLAMP.q2");
	wchar_t tempPath[MAX_PATH] = {0};
	GetTempPathW(MAX_PATH, tempPath);
	PathCombineW(TEMP_FILE, tempPath, L"WinLAMP.tmp");
	...
	if (INI_FILE[0])
	{
		...
		PathCombineW(INI_FILE, CONFIGDIR, L"WinLAMP.ini");
...
BOOL config_removedircontext(BOOL use_fallback)
{
	...
	{
		registrar->RemoveDirectoryContext(WINLAMP_PLAYW);
		registrar->RemoveDirectoryContext(WINLAMP_ENQUEUEW);
		registrar->RemoveDirectoryContext(WINLAMP_BOOKMARKW);
		registrar->Release();
		return TRUE;
	}
	return FALSE;
...
void config_agent_add(void)
{
	...
	if (GetRegistrar(&registrar, true) == 0 && registrar)
	{
		...
		PathCombineW(exe_name, exe_name, L"winlampa.exe");
...
void config_agent_remove(void)
{
	...
	if (GetRegistrar(&registrar, true) == 0 && registrar)
	{
		HWND hwnd = FindWindowW(L"WinLAMPAgentMain", NULL);
		if (hwnd)
		{
...
BOOL config_register_capability(wchar_t *ext, int mode)
{
	...
	if (GetRegistrar(&registrar, !mode) == 0 && registrar)
	{
		...
		if (FAILED(StringCchCopyExW(cursor, remaining, WINLAMP_FILEW, &cursor, &remaining, 0)))
			return TRUE;
