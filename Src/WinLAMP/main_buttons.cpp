...
{
	if (id == WINLAMP_BUTTON1_SHIFT)	SendMessageW(hwnd, WM_COMMAND, WINLAMP_REW5S, 0);
	else if (id == WINLAMP_BUTTON1_CTRL)
	{
...
int Main_OnButton2(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	...
	if (!playing)
	{
		...
		if (id == WINLAMP_BUTTON2_CTRL)
			SendMessageW(hwnd, WM_COMMAND, WINLAMP_FILE_LOC, 0);
		else if (!FileName[0] || id == WINLAMP_BUTTON2_SHIFT)
		{
			SendMessageW(hwnd, WM_COMMAND, WINLAMP_FILE_PLAY, 0);
		}
		else
		{
			StartPlaying();
	...
	else
	{
		if (id == WINLAMP_BUTTON2_CTRL)
			SendMessageW(hwnd, WM_COMMAND, WINLAMP_FILE_LOC, 0);
		else if (id == WINLAMP_BUTTON2_SHIFT)
			SendMessageW(hwnd, WM_COMMAND, WINLAMP_FILE_PLAY, 0);
...
int Main_OnButton4(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	if (playing)
	{
		if (id == WINLAMP_BUTTON4_CTRL)
		{
			HMENU m;
			int r;
			g_stopaftercur = !g_stopaftercur;
			r = g_stopaftercur ? MF_CHECKED : MF_UNCHECKED;
			CheckMenuItem(main_menu, WINLAMP_BUTTON4_CTRL, r);
			m = GetSubMenu(top_menu, 3);
			CheckMenuItem(m, WINLAMP_BUTTON4_CTRL, r);
			return 1;
		}
		if (id == WINLAMP_BUTTON4_SHIFT && !paused)
		{
//			double v=0;
			int x = 256;
...
{
	if (id == WINLAMP_BUTTON5_SHIFT)
		SendMessageW(hwnd, WM_COMMAND, WINLAMP_FFWD5S, 0);
	else if (id == WINLAMP_BUTTON5_CTRL)
	{
