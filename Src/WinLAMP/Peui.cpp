...
void peui_handlemouseevent(HWND hwnd, int x, int y, int type, int stats) 
{
	...
	if(!mouse_type)
	{
		...
		if(rs.bottom <= config_pe_height - 38)
		{
			...
			// TODO need to get the skinning code from gen_ml into winlamp.exe or split it out...
			static bool skinned;
			if(!skinned)
			{
...
static void do_titlebuttons() 
{
	...
	if (w) // kill button
	{
		if (mouse_type == -1 && which_cap == TB_CAP) 
		{
			...
			SendMessageW(hMainWindow,WM_COMMAND,w==1?WINLAMP_OPTIONS_PLEDIT:WINLAMP_OPTIONS_WINDOWSHADE_PL,0);
		}
