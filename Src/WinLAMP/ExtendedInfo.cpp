...
int in_get_extended_fileinfo(const char *fn, const char *metadata, char *dest, size_t destlen)
{
	...
	while (a >= 0)
	{
		...
		gefi = (GetANSIInfo)GetProcAddress(i->hDllInstance, "winlampGetExtendedFileInfo");
		gefiW = (GetUnicodeInfo)GetProcAddress(i->hDllInstance, "winlampGetExtendedFileInfoW");
		if (gefi || gefiW)
			break;
	}
...
int in_set_extended_fileinfo(const char *fn, const char *metadata, char *data)
{
	...
	while (a >= 0)
	{
		...
		sefi = (SetANSIInfo)GetProcAddress(i->hDllInstance, "winlampSetExtendedFileInfo");
		sefiW = (SetUnicodeInfo)GetProcAddress(i->hDllInstance, "winlampSetExtendedFileInfoW");
		wefi = (int (__cdecl *)())GetProcAddress(i->hDllInstance, "winlampWriteExtendedFileInfo");
		if (sefi || sefiW) break;
	}
