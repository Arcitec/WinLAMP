...
void pleditDlg(HWND hwnd, int init_state)
{
	...
	if (!config_pe_open)
	{
		CheckMenuItem(main_menu, WINLAMP_OPTIONS_PLEDIT, MF_CHECKED);
		if (pe_startuphack == 1)
		{
	...
	else
	{
		...
		{
			SendMessageW(hMainWindow, WM_COMMAND, WINLAMP_NEXT_WINDOW, 0);
		}
		CheckMenuItem(main_menu, WINLAMP_OPTIONS_PLEDIT, MF_UNCHECKED);
...
static int PE_OnLButtonDblClk(HWND hwnd, BOOL fDoubleClick, int x, int y, UINT keyFlags)
{
	...
	{
		SendMessageW(hMainWindow, WM_COMMAND, WINLAMP_OPTIONS_WINDOWSHADE_PL, 0);
	}
	else if (config_pe_height != 14)
	{
...
LRESULT CALLBACK PE_WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		...
		case WM_COMMAND:
			switch (LOWORD(wParam))
			{
				case ID_PE_SHOWPLAYING:
					...
					return 0;
				case WINLAMP_OPTIONS_WINDOWSHADE:
					SendMessageW(hMainWindow, WM_COMMAND, WINLAMP_OPTIONS_WINDOWSHADE_PL, 0);
					return 0;
				case ID_PE_FFOD:
				{
				...
				case ID_PE_SCDOWN:
					...
					{
						if (LOWORD(wParam) == ID_PE_SCUP)
							SendMessageW(hMainWindow, WM_COMMAND, WINLAMP_BUTTON1, 0);
						else
							SendMessageW(hMainWindow, WM_COMMAND, WINLAMP_BUTTON5, 0);
					}
					return 0;
		...
		case WM_MOUSEWHEEL:
		{
			...
			if (config_pe_height == 14)
			{
				if (dLines >= 0) dLines = WINLAMP_BUTTON1;
				else dLines = WINLAMP_BUTTON5;
			}
			else
			{
				if (0 != (MK_MBUTTON & LOWORD(wParam)))
