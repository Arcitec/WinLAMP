		...
		if (mouse_type == 1 || which_cap == SLID_CAP+which || (!which_cap && mouse_stats & MK_LBUTTON))
		{
			...
			*b = d;
			...
			} else 
			{
				...
				wchar_t HZStr[16] = {0};
				getStringW((which<5+!(config_eq_frequencies==EQ_FREQUENCIES_WINLAMP)?IDS_EQ_HZ:IDS_EQ_KHZ),HZStr,16);
				StringCchPrintfW(buf,128,L"EQ: %s%s: %s%0.01f %s",
								 ((config_eq_frequencies==EQ_FREQUENCIES_WINLAMP)?bands[which]:bandsISO[which]),
								 (!which?L"":HZStr),
								 v>=0.0?L"+":L"", v,
...
static void do_titlebuttons() 
{
	if (inreg(253,3,264+9,3+9)) // kill button
	{
		...
		if (mouse_type == -1 && which_cap == TB_CAP) 
		{
			which_cap=0;
			draw_eq_tbutton(0,0);
			if (ws==0) SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_OPTIONS_EQ,0);
			else SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_OPTIONS_WINDOWSHADE_EQ,0);
		}
		else if (mouse_stats & MK_LBUTTON)
		{
			which_cap=TB_CAP;
