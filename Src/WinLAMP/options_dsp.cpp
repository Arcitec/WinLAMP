...
static void DSPUpdateSel( HWND hwndDlg, HWND listWindow, INT iItem )
{
	...
	if ( ListView_GetNextItem( listWindow, -1, LVIS_SELECTED ) != -1 )
	{
		...
		if ( *libname && !skip )
		{
			...
			if ( hLib )
			{
				winlampDSPGetHeaderType pr = (winlampDSPGetHeaderType) GetProcAddress( hLib, "winlampDSPGetHeader2" );
				if ( pr )
				{
					int i = 0;
					...
					for ( ;;)
					{
						winlampDSPModule *module = pr( hMainWindow )->getModule( i++ );
				...
				do
				{
					if ( !g_safeMode )
					{
						...
						if ( hLib )
						{
							...
							if ( pr )
							{
								winlampDSPHeader *header = pr( hMainWindow );
		...
		switch ( LOWORD( wParam ) )
		{
			...
			case IDC_DSPCONF:
			{
				...
				if ( IsWindowEnabled( GetDlgItem( hwndDlg, IDC_DSPCONF ) ) )
				{
					...
					{
						winlampDSPGetHeaderType pr = (winlampDSPGetHeaderType) GetProcAddress( hLib, "winlampDSPGetHeader2" );
						winlampDSPModule *module = pr( hMainWindow )->getModule( SendDlgItemMessageA( hwndDlg, IDC_DSPMOD, CB_GETCURSEL, 0, 0 ) );
			...
			case IDC_PLUGINVERS:
				myOpenURLWithFallback( hwndDlg, L"http://www.google.com/search?q=WinLAMP+DSP+Effect+Plugins", L"http://www.google.com/search?q=WinLAMP+DSP+Effect+Plugins" );
				return TRUE;
		}
	}
