...
///////// if you update this, be sure to update the copy of it in $/winlampa/winlampicon.c
// thx.
int geticonid(int x)
{
	switch (x) {
...
void readwrite_client_uid(int isWrite, wchar_t uid_str[64])
{
	...
	if (RegCreateKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Nullsoft\\WinLAMP", 0, 0, 0, KEY_READ, NULL, &hkey, NULL) == ERROR_SUCCESS)
	{
