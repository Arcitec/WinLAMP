#include "main.h"
#include "WinLAMPPlaylist.h"
#include "../nu/AutoChar.h"
#include "../nu/AutoWide.h"
#include "api.h"

WinLAMPDirectoryLoad::WinLAMPDirectoryLoad( bool _forceRecurse, char *_exts ) : forceRecurse( _forceRecurse ), exts( _exts ), needsFree( false ), listStart( 0 )
{
...
WinLAMPDirectoryLoad::~WinLAMPDirectoryLoad()
{
	if ( config_rofiob & 1 )
...
WinLAMPPlaylist::WinLAMPPlaylist( const wchar_t *_base, bool _loadDirectories ) : base( _base ), terminateOnEnd( false ), loadDirectories( _loadDirectories )
{}
...
bool WhitelistOK( wchar_t *url )
{
	...
	{
		wchar_t *whitelist;
		whitelist = wcsstr( p, L"http://www.winlamp.com/" );
...
WinLAMPPlaylist::~WinLAMPPlaylist()
{
	//if ( terminateOnEnd )
	//{
	//}
...
void WinLAMPPlaylist::OnFile(const wchar_t *filename, const wchar_t *title, int lengthInMs, ifc_plentryinfo *info)
{
	BOOL hidden = false;
	int playcount = 0;
...
const wchar_t *WinLAMPPlaylist::GetBasePath()
{
	return base;
}

bool WinLAMPDirectoryLoad::ShouldRecurse(const wchar_t *path)
{
	if (config_rofiob&1)
		PlayList_sort(2, listStart);
...
bool WinLAMPDirectoryLoad::ShouldLoad(const wchar_t *filename)
{
	if (!exts)
		return true;
...
#define CBCLASS WinLAMPPlaylist
START_DISPATCH;
...
#undef CBCLASS
#define CBCLASS WinLAMPDirectoryLoad
START_DISPATCH;
