...
INT_PTR CALLBACK FtypeProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam) 
{
	...
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			if (FindWindowW(L"WinLAMPAgentMain",NULL))
				CheckDlgButton(hwndDlg,IDC_CHECK1,BST_CHECKED);
			if (GetPrivateProfileIntW(L"WinLAMPAgent",L"is_intray",1,INI_FILE))
		...
		case WM_DESTROY:
		{
			...
			hwnd = FindWindowW(L"WinLAMPAgentMain",NULL);
			...
			switch (LOWORD(wParam))
			{
				...
				case IDC_CHECK2:
				{
					...
					if (IsDlgButtonChecked(hwndDlg,IDC_CHECK2))
						WritePrivateProfileStringW(L"WinLAMPAgent",L"is_intray",L"1",INI_FILE);
					else
						WritePrivateProfileStringW(L"WinLAMPAgent",L"is_intray",L"0",INI_FILE);
