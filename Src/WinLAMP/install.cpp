...
void DoInstall(int is_install)
{
	...
	if (is_install&128)
	{
		...
		if (S_OK == hr)
		{
			...
			WritePrivateProfileStringW(L"WinLAMPReg", L"NeedReg", L"0", INI_FILE);
			...
			StartWinLAMP((NULL != hwndStatus), &hwndWA, szWAParam);	
		}
	...
	if (is_install&16) WritePrivateProfileStringW(L"WinLAMPReg", L"NeedReg", L"1", INI_FILE);
