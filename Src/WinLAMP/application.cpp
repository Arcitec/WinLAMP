 ...
 ** Project: WinLAMP 5
 ** Description: WinLAMP's implementation of Wasabi's Application API.
...
Application::DirectMouseWheel_ProccessMessage(MSG *msg)
{
	...
	else if (0 != SendMessageW(targetWindow, WINLAMP_WM_DIRECT_MOUSE_WHEEL, msg->wParam, msg->lParam))
		return true;
...
int Application::GetUserID(GUID *id)
{
	if (userID == GUID_NULL)
	{
		config_uid_ft = (char)GetPrivateProfileIntW(L"WinLAMP", L"uid_ft", 0, INI_FILE);
