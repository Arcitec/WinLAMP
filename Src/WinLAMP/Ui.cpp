...
{        
	int m = WINLAMP_BUTTON1 + which;
	if (which == 5)
	{
		if (mouse_stats & MK_CONTROL) SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_FILE_LOC,0);
		else if (mouse_stats & MK_SHIFT) SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_FILE_DIR,0);
		else SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_FILE_PLAY,0);
	}
	else
	{
...
static int do_clutterbar()
{
	...
	{
		en=2;
		if (mouse_type == -1) SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_OPTIONS_AOT,0);
		if (mouse_stats & MK_LBUTTON) 
		{
			wchar_t songnameStr[128] = {0};
			if (config_aot)
	...
	{
		en=4;
		if (mouse_type == -1) SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_OPTIONS_DSIZE,0);
		if (mouse_stats & MK_LBUTTON) 
		{
			wchar_t songnameStr[128] = {0};
...
static int do_titlebuttons() 
{
	...
	if (inreg(254,3,262,12)) // windowshade button
	{
		if (mouse_type == -1)
		{
			SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_OPTIONS_WINDOWSHADE,0);
...
static int do_peeq() 
{
	...
	if (inreg(219,58,219+23,58+12))  // eq
	{
		...
		case -1:
			do_eq_needupdate=0;
			SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_OPTIONS_EQ,0);
			break;
		}
		return 1;
	} 
	...
	if (inreg(219+23,58,219+23+23,58+12))  // pl
	{
		...
		case -1:
			do_pe_needupdate=0;
			SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_OPTIONS_PLEDIT,0);
...
static int do_shuffle() 
{
	...
	if (inreg(133+7-4+29,75+15,181+7-4-4+29,87+15)) 
	{
		...
		case -1:
			...
			SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_FILE_SHUFFLE,0);
			break;
		}
		return 1;
	} 
...
static int do_repeat() 
{
	...
	if (inreg(182+7-4-4+29,75+15,182+29+7-4-4+29,87+15)) 
	{
		...
		case -1:
			...
			SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_FILE_REPEAT,0);
			break;
		}
		return 1;
	} 
...
static int do_icon()
{
	...
	{
		if (mouse_type == -1) 
			SendMessageW(hMainWindow,WM_COMMAND,WINLAMP_LIGHTNING_CLICK,0);
		return 1;
	}
	return 0;
}
