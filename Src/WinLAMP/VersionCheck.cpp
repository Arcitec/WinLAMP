				...
				if (size >= 3 && p[1] == '.')
				{
					...
					if (verDif > 0) // same version or older
					{
						...
						if (size)
						{
							...
							{
								if (MessageBoxA(NULL, disp, getString(IDS_WINLAMP_UPDATE_MSG,NULL,0), MB_YESNO) == IDYES)
								{
...
static void CheckVersion(int allowedChannel = 0)
{
	if ( WAC_API_DOWNLOADMANAGER )
	{
		...
		wa::strings::wa_string l_winlamp_product_ver( STR_WINLAMP_PRODUCTVER );
		l_winlamp_product_ver.replaceAll( ",", "." );

		StringCchPrintfExA( url, 1024, &urlend, &urlend_size, 0, "http://client.winlamp.com/update/latest-version.php?v=%s", l_winlamp_product_ver.GetA().c_str() );
...
void newversioncheck(void)
{
	if (isInetAvailable())
	{
		...
		if (DoVerChk(config_newverchk2))
		{
			...
			StringCchPrintfExA(url, urlsize, &url, &urlsize, 0,"http://client.winlamp.com/update/client_session.php?v=%s",APP_VERSION);
