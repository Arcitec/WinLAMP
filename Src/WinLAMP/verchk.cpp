...
static DWORD WINAPI _Thread(void *p666)
{
  ...
  unsigned long (__stdcall *inet_addr)(const char FAR *cp );	
	...
	{
		lstrcpyn(connect_host,"www.winlamp.com",sizeof(connect_host)/sizeof(*connect_host));
	}
	connect_port=80;
  ...
  if ((sock = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP))==INVALID_SOCKET) 
	...
	}
	{
		...
		if (*proxy) 
		{
      StringCchPrintf(request_file,1024, "http://www.winlamp.com%s",rf_url);
		}
    else StringCchCopy(request_file,1024,rf_url);
		StringCchPrintf(send_string,2048,"GET %s HTTP/1.0\r\n"
							 "User-Agent: WinLAMP/%s\r\n"
                             "Host: www.winlamp.com\r\n"
...
//    MessageBox(NULL,buf,buf,0);
  ...
  if (g_nvck&2) stats_clear();
    ...
    if (g_nvck&1)
    {
		  if (MessageBox(NULL,obuf,getString(IDS_WINLAMP_UPDATE_MSG,NULL,0),MB_YESNO) == IDYES)
		  {
			  myOpenURL(NULL, L"http://www.winlamp.com/getwinlamp/");
		  }
		  else
		  {
			  if ((g_nvck&1)&&config_newverchk) config_newverchk=getDay();
