...
WinLAMPApi *WASABI_API_WINLAMP = 0;

Random random;
WinLAMPURIHandler winlamp_uri_handler;
...
public:
	WinLAMPServices() :
		...
		winlampFactory(WASABI_API_WINLAMP)
	{
		WASABI_API_SVC->service_register(&jsapi2_creator_factory);
		WASABI_API_LNG = langManager;
		uriHandlerFactory.Register(WASABI_API_SVC, &winlamp_uri_handler);
	}
	...
	Singleton<api_winlamp, WinLAMPApi>                  winlampFactory;
	
	SingletonServiceFactory<svc_urihandler, WinLAMPURIHandler> uriHandlerFactory;
	JSAPI2CreatorFactory jsapi2_creator_factory;
};

WinLAMPServices *services=0;
namespace Wasabi
{
	bool loaded = false;
}
...
void Wasabi_FindSystemServices()
{
	...
	if (sf)
		WINLAMP5_API_TAGZ = (api_tagz *)sf->getInterface();
...
{
	waServiceFactory *sf;
	if (WINLAMP5_API_TAGZ)
	{
		sf = WASABI_API_SVC->service_getServiceByGuid(tagzGUID);
		if (sf)
			sf->releaseInterface(WINLAMP5_API_TAGZ);
		WINLAMP5_API_TAGZ = 0;
	}
