...
LRESULT Main_OnIPC(HWND hwnd, int which, WPARAM data)
{
	...
	switch (which)
	{
		...
		case IPC_POP_DISABLE_EXIT:
			g_exit_disabled--;
			return g_exit_disabled;
		case IPC_REGISTER_WINLAMP_IPCMESSAGE:
		...
		case IPC_ENABLEDISABLE_ALL_WINDOWS:
			...
			return 0;
		case IPC_RESTARTWINLAMP:
		case IPC_RESTARTSAFEWINLAMP:
			{
				g_restartonquit = (1 + (which == IPC_RESTARTSAFEWINLAMP));
				Main_OnClose(hwnd);
			}
			return 0;
		case IPC_UPDTITLE:
		...
		//FG> 5/19/03 -- added an ipc call similar to WINLAMP_FILE_PLAY but with a parent HWND param, like OPENURLBOX
		case IPC_OPENFILEBOX:
			getNewFile(1, (HWND)data, 0);
			plEditRefresh();
			return 1;
		//FG> 5/19/03 -- added an ipc call similar to WINLAMP_FILE_DIR but with a parent HWND param, like OPENURLBOX
		case IPC_OPENDIRBOX:
			{
				BROWSEINFOW bi = {0};
		...
		case IPC_SET_SHUFFLE:
			...
			{
				SendMessageW(hwnd, WM_COMMAND, WINLAMP_FILE_SHUFFLE, 0);
			}
			break;
		case IPC_SET_REPEAT:
			if (!!config_repeat != !!data)
			{
				SendMessageW(hwnd, WM_COMMAND, WINLAMP_FILE_REPEAT, 0);
			}
			break;
		case IPC_SET_MANUALPLADVANCE:
			if ((!!config_pladv) != (!data))
			{
				SendMessageW(hwnd, WM_COMMAND, WINLAMP_FILE_MANUALPLADVANCE, 0);
			}
			break;
