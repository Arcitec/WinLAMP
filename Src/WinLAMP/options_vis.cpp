	...
	if (ListView_GetNextItem(listWindow, -1, LVIS_SELECTED) != -1)
	{
		...
		if (hLib)
		{
			...
			if (pr)
			{
				for(int i = 0;;)
				{
					winlampVisModule *l_module = pr(hMainWindow)->getModule(i++);
...
INT_PTR CALLBACK VisProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam) 
{
	...
	else if (uMsg == WM_COMMAND)
	{
		switch (LOWORD(wParam))
		{
			...
			case IDC_VISCONF:
				{
					if (IsWindowEnabled(GetDlgItem(hwndDlg, IDC_VISCONF)))
					{
						...
						if (which >= 0 && which < ListView_GetItemCount(listWindow))
						{
							...
							{
								winlampVisGetHeaderType pr = (winlampVisGetHeaderType) GetProcAddress(hLib,"winlampVisGetHeader");
								winlampVisModule *module = pr(hMainWindow)->getModule(SendDlgItemMessageA(hwndDlg,IDC_VISMOD,CB_GETCURSEL,0,0));
								if (module)
								{
				...
				vis_stop();
				{
					if (IsWindowEnabled(GetDlgItem(hwndDlg, IDC_UNINSTVIS)))
					{
						...
						if (hLib)
						{
							...
							*(void**)&pr = (void*)GetProcAddress(hLib,"winlampUninstallPlugin");
							...
							if (!ret)
							{
								...
								if (GetRegistrar(&registrar, true) == 0 && registrar)
								{
									if (registrar->DeleteItem(buf) != S_OK)
									{
										...
										_w_i("show_prefs", 33);
										PostMessageW(hMainWindow, WM_USER, 0, IPC_RESTARTWINLAMP);
									}
									else
			...
			case IDC_PLUGINVERS:
				myOpenURLWithFallback(hwndDlg, L"http://www.google.com/search?q=WinLAMP+Visualization+Plugins", L"http://www.google.com/search?q=WinLAMP+Visualization+Plugins");
				return TRUE;
		}
	}
