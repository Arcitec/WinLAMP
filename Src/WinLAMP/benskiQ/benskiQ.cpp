...
static double benskiQ_freqs[10]={ 70, 180, 320, 600, 1000, 3000, 6000, 12000, 14000, 16000 }; // winlamp style frequency table
...
void benskiQ_init()
{
	...
	for (int x=0;x<10;x++)
	{
		benskiQ[x].set_parameters((config_eq_frequencies==EQ_FREQUENCIES_WINLAMP)?benskiQ_freqs[x]:benskiQ_freqs_iso[x], 1.0, BENSKIQ_Q);
	}
}
...
void benskiQ_eq_set(char data[10])
{
	...
	for (int x = 0; x < 10; x ++)
	{
		benskiQ[x].set_parameters((config_eq_frequencies==EQ_FREQUENCIES_WINLAMP)?benskiQ_freqs[x]:benskiQ_freqs_iso[x], VALTOGAIN(data[x]), BENSKIQ_Q);
	}
...
void benskiQ_reset(int srate, int nch)
{
	...
	int x;
	if (config_eq_frequencies == EQ_FREQUENCIES_WINLAMP)
