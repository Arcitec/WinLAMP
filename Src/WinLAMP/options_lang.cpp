...
static void _setreadme(HWND hwndDlg, wchar_t* readme_only_wlz_extraction)
{
	...
	else SetDlgItemTextA(hwndDlg,IDC_EDIT1,"\tWinLAMP Default Language");
}
...
static void LangSwitchDelayText(HWND hwndDlg){
	if(config_langpack2[0]){
		...
		StringCchPrintfW(tmp,MAX_PATH,getStringW(IDS_WA_RESTART_FOR_WLZ_NEEDED,NULL,0),
						 (config_langpack2[0]=='<')?L"WinLAMP Default Language":config_langpack2);
...
{	
	...
	{
		case WM_INITDIALOG:
			link_startsubclass(hwndDlg, IDC_WINLAMPLINK);
		case WM_USER+50:
		{
		...
		case WM_COMMAND:
		{
			switch (LOWORD(wParam))
			{
				...
				case IDC_WINLAMPLINK:
				{
					myOpenURLWithFallback(hwndDlg, L"http://www.google.com/search?q=%22winlamp+language+packs%22+5.9", L"http://forums.winlamp.com/showthread.php?t=458120#lang");
					return 0;
				}
				case IDC_CHDIR:
				{
				...
				case IDC_SELBOX:
				{
					...
					if (HIWORD(wParam) == LBN_SELCHANGE)
					{
						...
						if(cur){
							...
							SetDlgItemTextA(hwndDlg,IDC_EDIT1,"\tWinLAMP Default Language");
						}
					}
					return FALSE;
				}
