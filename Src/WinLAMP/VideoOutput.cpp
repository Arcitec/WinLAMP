...
HMENU BuildPopupMenu()
{
	HMENU menu = GetSubMenu(GetSubMenu(top_menu, 3), 13);
	{
		...
		if (!menuset)
		{
			...
			InsertMenuA(menu, (UINT)-1, MF_BYPOSITION | MF_POPUP, (UINT_PTR)GetSubMenu(top_menu, 0), "WinLAMP");
			menuset = 1;
		}
	}
	updateTrackSubmenu();
		...
		userVideo(false)
{
	...
	wc.lpszClassName = L"WinLAMPVideoChild";
	...
	CreateWindowExW(0, wc.lpszClassName, L"WinLAMPVideoChildWindow",
...
LRESULT VideoOutput::WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		...
		case WM_KEYDOWN:
		{
			...
			if (wParam == ' ')
			{
				SendMessageW(hMainWindow, WM_COMMAND, WINLAMP_BUTTON3, 0); return 0;
			}
