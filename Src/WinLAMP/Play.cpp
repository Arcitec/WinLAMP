...
void Play(const wchar_t *playstring)
{
	...
	g_stopaftercur = 0;
	CheckMenuItem(main_menu, WINLAMP_BUTTON4_CTRL, MF_UNCHECKED);
	CheckMenuItem(GetSubMenu(top_menu, 3), WINLAMP_BUTTON4_CTRL, MF_UNCHECKED);
	...
	if (!g_quit)
	{
		...
		if (!no_notify_play)
		{
			...
			else
			{
				...
				// when receiving a 'stop' command and we try to send an update via JSAPI1_CurrentTitleChanged()
				if (!app_name || app_name && !*app_name || (unsigned int)(ULONG_PTR)app_name < 65536) BuildAppName();
				set_caption(0, L"%S %S", (!app_name || app_name && !*app_name ? "WinLAMP" : app_name), app_version_string);
			}
