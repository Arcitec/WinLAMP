...
int ShowVideoWindow(int init_state)
{
	...
	CheckMenuItem(main_menu, WINLAMP_OPTIONS_VIDEO, MF_CHECKED);
...
void HideVideoWindow(int autoStop)
{
	...
	CheckMenuItem(main_menu, WINLAMP_OPTIONS_VIDEO, MF_UNCHECKED);
	...
	{
		PostMessageW(hMainWindow, WM_COMMAND, WINLAMP_BUTTON4, 0);
	}
}
...
void Vid_Cmd(windowCommand *wc)
{
	...
	case VIDCMD_2X:
		...
		break;
	case VIDCMD_LIB:
		SendMessageW(videoGetHwnd(), WM_COMMAND, WINLAMP_VIDEO_TVBUTTON, 0);
		break;
	case VIDPOPUP_MISC:
	...
	case WM_KEYDOWN: case WM_KEYUP: case WM_SYSKEYDOWN: case WM_SYSKEYUP:
		if ((GetAsyncKeyState(VK_CONTROL)&0x8000) && wParam == VK_F4)
		{
			if (uMsg == WM_KEYDOWN || uMsg == WM_SYSKEYDOWN)
				SendMessageW(hMainWindow, WM_COMMAND, WINLAMP_OPTIONS_VIDEO, 0);
		}
...
extern "C"
{
	...
	HWND video_Create()
	{
		return CreateWindowExW(WS_EX_ACCEPTFILES, L"WinLAMP Video", getStringW(IDS_VIDEOCAPTION, NULL, 0), (WS_OVERLAPPED | WS_CLIPCHILDREN)&(~WS_CAPTION),
