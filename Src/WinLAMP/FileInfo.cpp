...
public:
	...
	{
		if (filename) {
			// In order to prevent jtfe 1.33 from crashing winlamp, we need to make sure that 
...
static INT_PTR CALLBACK FileInfo(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			AddUnifiedFileInfoPane aufip = (AddUnifiedFileInfoPane)GetProcAddress(p->in->hDllInstance, "winlampAddUnifiedFileInfoPane");
