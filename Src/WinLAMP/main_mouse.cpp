...
int Main_OnRButtonUp(HWND hwnd, int x, int y, UINT flags)
{
	...
	if (x < 266 && x > 105 && y < 35 && y > 24)
	{
		...
		CheckMenuItem(hmenu,WINLAMP_TOGGLE_AUTOSCROLL,(config_autoscrollname&1)?MF_CHECKED:MF_UNCHECKED);
		...
		(config_windowshade && x >= 129 && y >= 3 && x < 129+28 && y < 3+6))
	{
		...
		CheckMenuItem(hmenu,WINLAMP_OPTIONS_ELAPSED,config_timeleftmode?MF_UNCHECKED:MF_CHECKED);
		CheckMenuItem(hmenu,WINLAMP_OPTIONS_REMAINING,config_timeleftmode?MF_CHECKED:MF_UNCHECKED);		
	}
	...
	else if (x >= 164 && y >= 89 && x < 211 && y < 104)			/// shuffle
	{
		...
		CheckMenuItem(hmenu,WINLAMP_FILE_SHUFFLE,config_shuffle?MF_CHECKED:MF_UNCHECKED); 
	}	
	else if (x >= 211 && y >= 89 && x < 211+28 && y < 104)			/// repeat
	{
		hmenu = GetSubMenu(GetSubMenu(top_menu,3),10);
		CheckMenuItem(hmenu,WINLAMP_FILE_REPEAT,config_repeat?MF_CHECKED:MF_UNCHECKED); 
		CheckMenuItem(hmenu,WINLAMP_FILE_MANUALPLADVANCE,config_pladv?MF_UNCHECKED:MF_CHECKED); 
	}	
	else if (x >= 219 && y >= 58 && x < 219+(265-219)/2 && y < 70)			/// eq
	{
		hmenu = GetSubMenu(GetSubMenu(top_menu,3),11);
		CheckMenuItem(hmenu,WINLAMP_OPTIONS_EQ,config_eq_open?MF_CHECKED:MF_UNCHECKED); 
	}	
	else if (x >= 219+(265-219)/2 && y >= 58 && x < 265 && y < 70)			/// pe
	{
		hmenu = GetSubMenu(GetSubMenu(top_menu,3),12);
		CheckMenuItem(hmenu,WINLAMP_OPTIONS_PLEDIT,config_pe_open?MF_CHECKED:MF_UNCHECKED); 
	}	
...
int Main_OnLButtonDblClk(HWND hwnd, BOOL fDoubleClick, int x, int y, UINT keyFlags)
{
	...
	{
		SendMessageW(hwnd,WM_COMMAND,WINLAMP_EDIT_ID3,0);
	}
	...
	{
		SendMessageW(hwnd,WM_COMMAND,WINLAMP_OPTIONS_WINDOWSHADE,0);
		UpdateWindow(hwnd);
		Sleep(200);
		for (;;)
		{
