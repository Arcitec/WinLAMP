...
LRESULT Main_OnCommand(HWND hwnd, int wID, HWND hwndCtl, UINT codeNotify)
{
	...
	if (LOWORD(wID) >= 32767 && LOWORD(wID) < g_SkinTop)
	{
		...
		if (_wcsicmp(config_skin, curfilename))
		{
			...
			SendMessageW(hMainWindow, WM_COMMAND, WINLAMP_REFRESHSKIN, 0);
	...
	case ID_HELP_HELPTOPICS:
		myOpenURL(hwnd, L"https://help.winlamp.com/hc/sections/8103009633300-WinLAMP-Desktop-Player-");
		break;
	case ID_HELP_FEEDBACK:
	{
		...
		StringCchPrintfW(url, 512, L"http://services.winlamp.com/redirect/support?reason=Feedback&subject=WinLAMP Feedback&product=WinLAMP Desktop&v=%s Build %d&platform=%s (%s)",
	...
	case ID_PE_SAVEAS:
		...
		return 1;
	case WINLAMP_EDIT_BOOKMARKS:
		MessageBoxW(hwnd, getStringW(IDS_ML_MISSING_FOR_BOOKMARKS,NULL,0), L"WinLAMP", MB_OK);
		return 1;
	case WINLAMP_MAKECURBOOKMARK:
		{
		...
		return 1;
	case WINLAMP_LIGHTNING_CLICK: about_dialog(); return 1;
	case WINLAMP_NEW_INSTANCE:
		{
			char buf[MAX_PATH] = "\"";
	...
	case WINLAMP_REFRESHSKIN:
		...
		return 1;
	case WINLAMP_SELSKIN:
		prefs_last_page = 40;
		prefs_dialog(1);
		return 1;
	case WINLAMP_VISPLUGIN:
		if (vis_running()) vis_stop();
		else vis_start(hMainWindow, NULL);
		return 1;
	case WINLAMP_PLGSETUP:
		prefs_last_page = 33;
		prefs_dialog(1);
		return 1;
	case WINLAMP_TOGGLE_AUTOSCROLL:
		...
		return 1;
	case WINLAMP_EDIT_ID3:
		if (FileName[0]) in_infobox(DIALOG_PARENT(hMainWindow), FileName);
		return 1;
	case WINLAMP_OPTIONS_WINDOWSHADE_PL:         // toggle windowshade
		ToggleWindowShade_PL();
		return 1;
	case WINLAMP_OPTIONS_WINDOWSHADE_EQ:
		ToggleWindowShade_EQ();
		return 1;
	case WINLAMP_OPTIONS_WINDOWSHADE:         // toggle windowshade
		ToggleWindowShade_Main();
		return 1;
	case WINLAMP_OPTIONS_WINDOWSHADE_GLOBAL:         // toggle windowshade
		{
	...
	case WINLAMP_MAIN_WINDOW:
		...
		//sa_setthread(-1);
		CheckMenuItem(main_menu, WINLAMP_MAIN_WINDOW, config_mw_open ? MF_CHECKED : MF_UNCHECKED);
		set_aot(0);

		Ipc_WindowToggle(IPC_CB_WND_MAIN, config_mw_open);

	case WINLAMP_NEXT_WINDOW:
...
//else if (x == 3) { /*dockwnd = hMBWindow; vis = 0;*/ } // config_mb_open; }
		...
		return 1;
	case WINLAMP_HELP_ABOUT:         // about box
		about_dialog();
		return 1;
	case WINLAMP_PE_SEARCH:
	case WINLAMP_JUMPFILE:
		jump_file_dialog(hMainWindow);
		return 1;
	case WINLAMP_TOGGLE_LIBRARY:
	...
	case WINLAMP_JUMP:
		...
		return 1;
	case WINLAMP_FFWD5S:         // left and right arrows, fastforward and rewind by 5 seconds
	case WINLAMP_REW5S:
		{
			int i;
			if (!in_mod || !in_mod->is_seekable || PlayList_ishidden(PlayList_getPosition())) return 1;
			if (LOWORD(wID) == WINLAMP_FFWD5S) i = 5000;
			else i = -5000;
			if (playing)
			{
				int t;
		...
		return 1;
	case WINLAMP_MAINMENU:         // alt-f for mainmenu
		{
			POINT p = { 6, 13 };
			if (config_dsize)
			{
	...
	case WINLAMP_PREVSONG:
		...
		return 0;
	case WINLAMP_BUTTON1_SHIFT:         // button 1 (previous), sent from either windowshade,
	case WINLAMP_BUTTON1_CTRL:          // keyboard shortcut, or normal buttons (ui.c)
	case WINLAMP_BUTTON1:
		return (Main_OnButton1(hwnd, LOWORD(wID), hwndCtl, codeNotify));
	case WINLAMP_BUTTON2_SHIFT:         // button 2 (play)
	case WINLAMP_BUTTON2_CTRL:
	case WINLAMP_BUTTON2:
		return (Main_OnButton2(hwnd, LOWORD(wID), hwndCtl, codeNotify));
	case WINLAMP_BUTTON3:               // button 3 (pause)
		return (Main_OnButton3(hwnd, LOWORD(wID), hwndCtl, codeNotify));
	case WINLAMP_BUTTON4_CTRL:         // button 4 (stop)
	case WINLAMP_BUTTON4_SHIFT:         // button 4 (stop)
	case WINLAMP_BUTTON4:
		return (Main_OnButton4(hwnd, LOWORD(wID), hwndCtl, codeNotify));
	case WINLAMP_BUTTON5_CTRL:         // button 5 (next)
	case WINLAMP_BUTTON5_SHIFT:         // button 5 (next)
	case WINLAMP_BUTTON5:
		return (Main_OnButton5(hwnd, LOWORD(wID), hwndCtl, codeNotify));
	case WINLAMP_FILE_QUIT:          // keyboard Alt-X or menu option
		if (g_exit_disabled) return 0;
		SendMessageW(hwnd, WM_CLOSE, 0, 0);
		return 1;
	case WINLAMP_FILE_LOC:          // open location
		getNewLocation(1, DIALOG_PARENT(hwnd));
		plEditRefresh();
		return 1;
	case WINLAMP_FILE_PLAY:         // open file(s)
		getNewFile(1,        /*NULL*/DIALOG_PARENT(hwnd), 0);
		plEditRefresh();
		return 1;
	case WINLAMP_FILE_DIR:         // open directory
		{
			BROWSEINFOW bi = {0};
		...
		return 1;
	case WINLAMP_OPTIONS_PREFS:         // open preferences
		prefs_dialog(0);
		return 1;
	case WINLAMP_OPTIONS_AOT:         // toggle always on top
		config_aot = !config_aot;
		set_aot(0);
		return 1;
	case WINLAMP_OPTIONS_DSIZE:         // toggle doublesize mode
		config_dsize = !config_dsize;
		if (!config_bifont && config_bifont_alt) draw_reinit_plfont(1);
		set_aot(1);
		return 1;
	case WINLAMP_OPTIONS_EASYMOVE:         // toggle easymove (easy window moving)
		config_easymove = !config_easymove;
		CheckMenuItem(main_menu, WINLAMP_OPTIONS_EASYMOVE, config_easymove ? MF_CHECKED : MF_UNCHECKED);
		return 1;
	case WINLAMP_FILE_SHUFFLE:         // toggle shuffle (and draw button)
		config_shuffle = !config_shuffle;
		CheckMenuItem(main_menu, WINLAMP_FILE_SHUFFLE, config_shuffle ? MF_CHECKED : MF_UNCHECKED);
		...
		return 1;
	case WINLAMP_FILE_REPEAT:         // toggle repeat
	{
		config_repeat = !config_repeat;
		CheckMenuItem(main_menu, WINLAMP_FILE_REPEAT, config_repeat ? MF_CHECKED : MF_UNCHECKED);
	...
	case WINLAMP_FILE_MANUALPLADVANCE:         // toggle manual playlist advance
	{
	...
	case WINLAMP_OPTIONS_EQ:         // toggle EQ window
		eq_dialog(hwnd,0);
		return 1;
	case WINLAMP_OPTIONS_PLEDIT:         // toggle playlist editor
		// if we get a 2 then it came from gen_ff so that we can work around a docking/position issue
		pleditDlg(hwnd,codeNotify==2);
		return 1;
	case WINLAMP_OPTIONS_VIDEO:
		config_video_open ? HideVideoWindow(1) : ShowVideoWindow(0);
		return 1;
	case WINLAMP_OPTIONS_TOGTIME:
		config_timeleftmode = !config_timeleftmode;
		CheckMenuItem(main_menu, WINLAMP_OPTIONS_ELAPSED, config_timeleftmode ? MF_UNCHECKED : MF_CHECKED);
		CheckMenuItem(main_menu, WINLAMP_OPTIONS_REMAINING, config_timeleftmode ? MF_CHECKED : MF_UNCHECKED);
		SendMessageW(hwnd, WM_TIMER, UPDATE_DISPLAY_TIMER + 4, 0);
		return 1;
	case WINLAMP_OPTIONS_ELAPSED:         // set time mode to elapsed
		config_timeleftmode = 0;
		CheckMenuItem(main_menu, WINLAMP_OPTIONS_ELAPSED, MF_CHECKED);
		CheckMenuItem(main_menu, WINLAMP_OPTIONS_REMAINING, MF_UNCHECKED);
		SendMessageW(hwnd, WM_TIMER, UPDATE_DISPLAY_TIMER + 4, 0);
		return 1;
	case WINLAMP_OPTIONS_REMAINING:         // set time mode to remaining
		config_timeleftmode = 1;
		CheckMenuItem(main_menu, WINLAMP_OPTIONS_ELAPSED, MF_UNCHECKED);
		CheckMenuItem(main_menu, WINLAMP_OPTIONS_REMAINING, MF_CHECKED);
		SendMessageW(hwnd, WM_TIMER, UPDATE_DISPLAY_TIMER + 4, 0);
		return 1;
	case WINLAMP_VOLUMEUP:		// increase volume by ~2%
	case WINLAMP_VOLUMEDOWN:		// decrease volume by ~2%
		if (LOWORD(wID) == WINLAMP_VOLUMEUP)
		{
		...
		return 1;
	case WINLAMP_JUMP10BACK:
		{
			int x;
			int i = playing;
			if (i) StopPlaying(0);
			for (x = 0; x < 10; x ++)
				SendMessageW(hwnd, WM_COMMAND, WINLAMP_BUTTON1, 0);
			if (i) StartPlaying();
		}
		return 1;
	case WINLAMP_JUMP10FWD:
		{
			int x;
			int i = playing;
			if (i) StopPlaying(0);
			for (x = 0; x < 10; x ++)
				SendMessageW(hwnd, WM_COMMAND, WINLAMP_BUTTON5, 0);
			if (i) StartPlaying();
		}
		return 1;
	case WINLAMP_GETMORESKINS:
		myOpenURLWithFallback(hwnd, L"http://www.google.com/search?q=WinLAMP+Skins", L"http://www.google.com/search?q=WinLAMP+Skins");
		return 0;
	case WINLAMP_VISCONF:
		{
			...
			if (hLib)
			{
				winlampVisModule *module = NULL;
				winlampVisGetHeaderType pr = (winlampVisGetHeaderType) GetProcAddress(hLib, "winlampVisGetHeader");
				winlampVisHeader* pv = pr(hMainWindow);
		...
		return 1;
	case WINLAMP_MINIMIZE:
		if (GetAsyncKeyState(VK_SHIFT) >> 15)
			minimize_hack_winlamp = 1;
		else
			minimize_hack_winlamp = 0;
		ShowWindow(hMainWindow, SW_MINIMIZE);
		return 1;
	case WINLAMP_VIDEO_TOGGLE_FS:
...
#define TREE_INTERNET_VIDEO 11001
	...
	case ID_PLAY_VOLUMEUP:          // increase by ~ 10%
		{
			int a = 5;
			while (a--) SendMessageW(hwnd, WM_COMMAND, WINLAMP_VOLUMEUP, 0);
		}
		return 1;
	case ID_PLAY_VOLUMEDOWN:         // decrease by ~10%
		{
			int a = 5;
			while (a--)
				SendMessageW(hwnd, WM_COMMAND, WINLAMP_VOLUMEDOWN, 0);
		}
		return 1;
