...
/* register winlamp as a media player (in "set defaults")*/
...
BOOL config_adddircontext(BOOL use_fallback)
{
	...
	if (GetRegistrar(&registrar, !use_fallback) == 0 && registrar)
	{
		...
		registrar->AddDirectoryContext(str, WINLAMP_PLAYW, getStringW(IDS_CONFIG_PLAYSTR,langbuf,1024));

		StringCchPrintfW(str,MAX_PATH+128,L"\"%s\" /ADD \"%%1\"",programname);
		registrar->AddDirectoryContext(str, WINLAMP_ENQUEUEW, getStringW(IDS_CONFIG_ENQUEUESTR,langbuf,1024));

		StringCchPrintfW(str,MAX_PATH+128,L"\"%s\" /BOOKMARK \"%%1\"",programname);
		registrar->AddDirectoryContext(str, WINLAMP_BOOKMARKW, getStringW(IDS_CONFIG_BOOKMARKSTR,langbuf,1024));
...
int config_isdircontext(void)
{
	if (config_no_registry)
		return 0;
	HKEY mp3Key;
	if (RegOpenKeyA(HKEY_LOCAL_MACHINE,"SOFTWARE\\Classes\\Directory\\shell\\" WINLAMP_PLAY,&mp3Key) != ERROR_SUCCESS) return 0;
	RegCloseKey(mp3Key);
	return 1;
}

BOOL config_setup_filetype(const wchar_t *winlamp_file, const wchar_t *name, BOOL use_fallback)
{
	wchar_t programname[MAX_PATH] = {0};
	if (!GetModuleFileNameW(hMainInstance,programname,MAX_PATH)) return FALSE;
	...
	if (GetRegistrar(&registrar, use_fallback) == 0 && registrar)
	{
		...
		registrar->SetupFileType(programname, winlamp_file, name, config_whichicon, config_addtolist?L"Enqueue":L"Play", L"");

		StringCchPrintfW(str,MAX_PATH+32,L"\"%s\" \"%%1\"",programname);
		registrar->SetupShell(str, winlamp_file, getStringW(IDS_CONFIG_PLAYSTR,langbuf,1024), L"Play", L"{46986115-84D6-459c-8F95-52DD653E532E}");

		StringCchPrintfW(str,MAX_PATH+32,L"\"%s\" \"%%1\"",programname);
		registrar->SetupShell(str, winlamp_file, L"", L"open", L"{46986115-84D6-459c-8F95-52DD653E532E}");

		StringCchPrintfW(str,MAX_PATH+32,L"\"%s\" /ADD \"%%1\"",programname);
		registrar->SetupShell(str, winlamp_file, getStringW(IDS_CONFIG_ENQUEUESTR,langbuf,1024), L"Enqueue", L"{77A366BA-2BE4-4a1e-9263-7734AA3E99A2}");

		StringCchPrintfW(str,MAX_PATH+32,L"\"%s\" /BOOKMARK \"%%1\"",programname);
		registrar->SetupShell(str, winlamp_file, getStringW(IDS_CONFIG_BOOKMARKSTR,langbuf,1024), L"ListBookmark", L"");
...
BOOL config_setup_filetypes(int mode)
{
	...
	if (!config_setup_filetype(WINLAMP_FILEW, getStringW(IDS_WINLAMP_MEDIA_FILE,NULL,0), !mode))
		return FALSE;
	...
	if (GetRegistrar(&registrar, !mode) == 0 && registrar)
	{
		...
		registrar->RegisterTypeShell(programname, WINLAMP_PLAYLISTW, getStringW(IDS_WINLAMP_PLAYLIST_FILE,NULL,0), config_whichicon2, config_addtolist?L"Enqueue":L"Play");

		StringCchPrintfW(str,MAX_PATH+32,L"\"%s\" \"%%1\"",programname);
		registrar->SetupShell(str, WINLAMP_PLAYLISTW, getStringW(IDS_CONFIG_PLAYSTR,langbuf,1024), L"Play", L"");

		StringCchPrintfW(str,MAX_PATH+32,L"\"%s\" \"%%1\"",programname);
		registrar->SetupShell(str, WINLAMP_PLAYLISTW, L"", L"open", L"");

		StringCchPrintfW(str,MAX_PATH+32,L"\"%s\" /ADD \"%%1\"",programname);
		registrar->SetupShell(str, WINLAMP_PLAYLISTW, getStringW(IDS_CONFIG_ENQUEUESTR,langbuf,1024), L"Enqueue", L"");

		StringCchPrintfW(str,MAX_PATH+32,L"\"%s\" /BOOKMARK \"%%1\"",programname);
		registrar->SetupShell(str, WINLAMP_PLAYLISTW, getStringW(IDS_CONFIG_BOOKMARKSTR,langbuf,1024), L"ListBookmark", L"");

		/* Register skin shell type */
		registrar->RegisterTypeShell(programname, WINLAMP_SKINZIPW, getStringW(IDS_WINLAMP_EXTENSION_INSTALLATION_FILE,NULL,0), config_whichicon, L"Install");

		StringCchPrintfW(str,MAX_PATH+32,L"\"%s\" \"%%1\"",programname);
		registrar->SetupShell(str, WINLAMP_SKINZIPW, getStringW(IDS_WINLAMP_FILE_INSTALL,langbuf,1024), L"Install", L"");

		/* Register language file type */
		registrar->RegisterTypeShell(programname, WINLAMP_LANGZIPW, getStringW(IDS_WINLAMP_LANGUAGE_INSTALLATION_FILE,NULL,0), config_whichicon, L"Install");

		StringCchPrintfW(str,MAX_PATH+32,L"\"%s\" \"%%1\"",programname);
		registrar->SetupShell(str, WINLAMP_LANGZIPW, getStringW(IDS_WINLAMP_FILE_INSTALL,langbuf,1024), L"Install", L"");

		regmimetype(L"interface/x-winlamp-skin", programname, L".wsz", 0);
		regmimetype(L"interface/x-winlamp-lang", programname, L".wlz", 0);
		...
		//  regmimetype(L"application/x-winlamp-plugin", programname,L"wpz",0);
		registrar->Release();
		return TRUE;
	}
	return FALSE;
...
{
	wchar_t winlampPath[MAX_PATH] = {0};

	// get the full path exe
	if (!GetModuleFileNameW(hMainInstance,winlampPath, MAX_PATH))
		return;

	IFileTypeRegistrar *registrar=0;
	if (GetRegistrar(&registrar, true) == 0 && registrar)
	{
		registrar->RegisterDVDPlayer(winlampPath, config_whichicon, L"WinLAMP.DVD", L"WinLAMP", L"Nullsoft WinLAMP", L"Play in WinLAMP");
		registrar->Release();
	}
}
