...
// attempt to cleanup the last extracted temp folder for a skin incase WinLAMP crashed on exit
...
//		;
...
#if 0
		{
			...
			GetPrivateProfileString("WinLAMP", "MBOpen", "", str, sizeof(str), fn);
			str[1023] = 0;
			if (str[0])
			{
				if (hMBWindow)
...
void Skin_Random(void)
{
	if (config_randskin)
	{
		...
		PostMessageW(hMainWindow, WM_COMMAND, WINLAMP_REFRESHSKIN, 0);
