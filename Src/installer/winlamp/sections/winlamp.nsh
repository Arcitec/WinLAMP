...
${WinLAMPSection} "winlampApplication" $(secWinLAMP) IDX_SEC_WINLAMP           ; <<< [WinLAMP]
	...
	${DllUnregisterAndDelete} "$INSTDIR\Plugins\cddbcontrolwinlamp.dll"
	${DllUnregisterAndDelete} "$INSTDIR\Plugins\cddbuiwinlamp.dll"
	${DllUnregisterAndDelete} "$INSTDIR\Plugins\Gracenote\cddbuiwinlamp.dll"
	${DllUnregisterAndDelete} "$INSTDIR\Plugins\Gracenote\CddbMusicIDWinLAMP.dll"
	${DllUnregisterAndDelete} "$INSTDIR\Plugins\Gracenote\CddbPlaylist2WinLAMP.dll"
	${DllUnregisterAndDelete} "$INSTDIR\Plugins\Gracenote\cddbcontrolwinlamp.dll"
	...
	Delete "$SETTINGSDIR\winlamp.pic" ; deprecated plug-in cache file
	...
	; Delete DrO's ml_enqplay plugin because it crashes WinLAMP 5.9
	...
	; Delete DrO's gen_find_on_disk.dll which is now fully integrated into WinLAMP in 5.64
	...
	; Delete DrO's gen_cd_menu.dll which is now fully integrated into WinLAMP in 5.64
	; The functionality was in winlamp agent (showing cd drive volume name in menus)
	...
	; Delete DrO's gen_mwblock.dll which is now fully integrated into WinLAMP in 5.66
	...
	; Disable 3rd-party gen_lyrics_ie plugin (causes WinLAMP to crash)
	...
	DeleteINIStr "$WINLAMPINI" "WinLAMP" "mbdefloc"
...
;!ifdef PRO
;	WriteINIStr "$WINLAMPINI" "WinLAMP" "PromptForRegKey" "1"
;!endif ; PRO
	SetOutPath "$INSTDIR"

	SetOverwrite off
	File "/oname=$WINLAMPM3U" "..\..\resources\media\winlamp.m3u"
	SetOverwrite ${OVERWRITEMODE}

	File "/oname=${WINLAMPEXE}" "${FILES_PATH}\winlamp.exe"
	...
	RMDir /r "$R0\WinLAMP Bento"
	Delete "$R0\WinLAMP Bento.*"
...
;!endif
	...
	; bugfix?
	; this might come back due to WinLAMPURIHandler reg code in WinLAMP\handler.cpp
	; which can cause filetypes associated with WinLAMP to be erroneously registered as "URL:WinLAMP Command Handler"
	DeleteRegKey HKCR "winlamp"
	...
	ExecDos::exec /NOUNLOAD /TIMEOUT=5000 'netsh advfirewall firewall set rule name="${WINLAMP}" dir=in program="$0" profile=private,public protocol=TCP new action=allow enable=yes' "" ""
	Pop $1 ;return value
	${If} $1 != 0 ; failed
		; attempt to add application using advanced method (Vista+) 
		ExecDos::exec /NOUNLOAD /TIMEOUT=5000 'netsh advfirewall firewall add rule name="${WINLAMP}" dir=in action=allow program="$0" enable=yes profile=private,public protocol=TCP' "" ""
		Pop $1
		${If} $1 != 0 
			ExecDos::exec /NOUNLOAD /TIMEOUT=2000 'netsh firewall add allowedprogram program="$0" name="${WINLAMP}" mode=ENABLE scope=ALL profile=ALL' "" ""
		${EndIf}
	${EndIf}

	; UDP
	; try to update existing rule
	ExecDos::exec /NOUNLOAD /TIMEOUT=5000 'netsh advfirewall firewall set rule name="${WINLAMP}" dir=in program="$0" profile=private,public protocol=UDP new action=allow enable=yes' "" ""
	Pop $1 ;return value
	${If} $1 != 0 ; failed
		; attempt to add application using advanced method (Vista+) 
		ExecDos::exec /NOUNLOAD /TIMEOUT=5000 'netsh advfirewall firewall add rule name="${WINLAMP}" dir=in action=allow program="$0" enable=yes profile=private,public protocol=UDP' "" ""
	${EndIf}

	Pop $1
	Pop $0
	;SetDetailsPrint lastused
${WinLAMPSectionEnd} ;                                                                   <<< [WinLAMP]
