SectionGroup $(IDS_GRP_UIEXTENSION) IDX_GRP_UIEXTENSION ;  User Interface Extensions
	${WinLAMPSection} "GlobalHotkeys" $(secHotKey) IDX_SEC_HOTKEY      ; >>> [Global Hotkey Support]
		${SECTIONIN_LITE}
		SetOutPath $INSTDIR\Plugins
		File ${FILES_PATH}\Plugins\gen_hotkeys.dll
	${WinLAMPSectionEnd}                                               ; <<< [Global Hotkey Support]

	${WinLAMPSection} "TrayControl" $(secTray) IDX_SEC_TRAYCTRL        ; >>> [Nullsoft Tray Control]
		${SECTIONIN_LITE}
		SetOutPath $INSTDIR\Plugins
		File ${FILES_PATH}\Plugins\gen_tray.dll
	${WinLAMPSectionEnd}                                               ; <<< [Nullsoft Tray Control]

!ifndef WINLAMP64
!ifdef full | std
	${WinLAMPSection} "FreeformSkins" $(compModernSkin) IDX_SEC_FREEFORM      ; >>> [Modern Skin Support]
		${SECTIONIN_STD}
		...
		ReadIniStr $0 "$WINLAMPINI" "WinLAMP" "skin"
		${If} "$0" == "WinLAMP Bento"
			WriteIniStr "$WINLAMPINI" "WinLAMP" "skin" "Bento"
		${EndIf}
	${WinLAMPSectionEnd}                                            ; <<< [Modern Skin Support]
!endif ; full | std
!endif ; WINLAMP64
