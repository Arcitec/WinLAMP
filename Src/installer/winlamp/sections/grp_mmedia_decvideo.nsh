...
SectionGroup $(IDS_GRP_MMEDIA_VIDEO_DEC)  IDX_GRP_MMEDIA_VIDEO_DEC ; Video Playback
	${WinLAMPSection} "decoderWmv" $(secWMV) IDX_SEC_WMV_DEC
		${SECTIONIN_STD}
		SetOutPath $INSTDIR\Plugins
		File ${FILES_PATH}\Plugins\in_wm.dll
		ClearErrors
		ReadINIStr $0 "$WINLAMPINI" "in_dshow" "extlist"
		${IfNot} ${Errors}
			${If} $0 <> ""
				...
				Pop $0
				WriteINIStr "$WINLAMPINI" "in_dshow" "extlist" $0
			${EndIf}
		${EndIf}
	${WinLAMPSectionEnd}
   
!ifndef WINLAMP64
	${WinLAMPSection} "decoderNsv" $(secNSV) IDX_SEC_NSV_DEC
		${SECTIONIN_STD}
		SetOutPath $INSTDIR\Plugins
...
;		File /nonfatal ${FILES_PATH}\System\vp8.wbm
		...
		WriteRegStr HKEY_CLASSES_ROOT "UNSV\shell\open\command" "" "$INSTDIR\${WINLAMPEXE} %1"
	${WinLAMPSectionEnd}
!endif ; WinLAMP64

!ifndef WINLAMP64
	${WinLAMPSection} "decoderDirectShow" $(secDSHOW) IDX_SEC_DSHOW_DEC
		${SECTIONIN_STD}
		SetOutPath $INSTDIR\Plugins
		File ${FILES_PATH}\Plugins\in_dshow.dll
		WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Explorer\AutoplayHandlers\EventHandlers\PlayVideoFilesOnArrival" "${WINLAMP}PlayMediaOnArrival" ""
	${WinLAMPSectionEnd}
!endif ; WINLAMP64
   
	${WinLAMPSection} "decoderAvi" $(secAVI) IDX_SEC_AVI_DEC
		${SECTIONIN_STD}
		SetOutPath $INSTDIR\Plugins
...
;		File /nonfatal ${FILES_PATH}\System\mp4v.wbm
		...
		; AVI audio codecs
		; aacdec.w5s is installed by default (see winlamp.nsh)
...
;		File /nonfatal ${FILES_PATH}\System\pcm.wbm
	...
	${WinLAMPSection} "decoderFlv" $(secFLV) IDX_SEC_FLV_DEC
		${SECTIONIN_STD}
		SetOutPath $INSTDIR\Plugins
...
;		File /nonfatal ${FILES_PATH}\System\f263.wbm
	...
	${WinLAMPSection} "decoderMkv" $(secMKV) IDX_SEC_MKV_DEC
		${SECTIONIN_STD}
		SetOutPath $INSTDIR\Plugins
...
;		File /nonfatal ${FILES_PATH}\System\f263.wbm
	...
	${WinLAMPSection} "decoderM4v" $(secM4V) IDX_SEC_M4V_DEC
		${SECTIONIN_STD}
		SetOutPath $INSTDIR\Plugins
...
!ifndef WINLAMP64
	${WinLAMPSection} "decoderSwf" $(secSWF) IDX_SEC_SWF_DEC
		${SECTIONIN_FULL}
		SetOutPath $INSTDIR\Plugins
		File ${FILES_PATH}\Plugins\in_swf.dll
		File ..\..\resources\data\winlampFLV.swf
	${WinLAMPSectionEnd}
!endif ; WINLAMP64
