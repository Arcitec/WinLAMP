!ifndef NULLSOFT_WINLAMP_INSTALLER_HELPER_FUNCTIONS_HEADER
!define NULLSOFT_WINLAMP_INSTALLER_HELPER_FUNCTIONS_HEADER
...
!macro PromptCloseProgram UNIQUEBS CLASSNAME
	...
	${UNIQUEBS}loop:
		FindWindow $R0 "${CLASSNAME}"
		IntCmp $R0 0 ${UNIQUEBS}done
		MessageBox MB_DEFBUTTON2|MB_ABORTRETRYIGNORE $(msgCloseWinLAMP) IDIGNORE ${UNIQUEBS}done IDRETRY ${UNIQUEBS}skipabort
...
Function SetVisPluginDir
	...
	ReadINIStr $R0 "$WINLAMPINI" "WinLAMP" "VISDIR"
...
Function GetSkinDir
	Push $R0
	ReadINIStr $R0 "$WINLAMPINI" "WinLAMP" "SKINDIR"
	StrCmp $R0 "" "" NoSkinCh
...
Function SetupWinLAMPDirectories
	...
	${If} ${FileExists} "$INSTDIR\paths.ini"
		Push $INSTDIR
		nsis_winlamp::ReadSettings
		nsis_winlamp::GetIniPath
		Pop $SETTINGSDIR
		nsis_winlamp::GetIniFile
		Pop $WINLAMPINI
		nsis_winlamp::GetM3uPath
		Pop $WINLAMPM3U
		nsis_winlamp::GetM3uBase
		Pop $M3UBASEDIR

		${If} $WINLAMPINI == ""
			StrCpy $WINLAMPINI "winlamp.ini"
		${EndIf}
	${Else}
		StrCpy $SETTINGSDIR ""
		StrCpy $WINLAMPINI "winlamp.ini"
	...
	${If} $WINLAMPM3U == ""
		StrCpy $WINLAMPM3U "$SETTINGSDIR"
	${EndIf}
  
	${If} $M3UBASEDIR == ""
		StrCpy $M3UBASEDIR "$WINLAMPM3U"
	${EndIf}
...
!endif ;NULLSOFT_WINLAMP_INSTALLER_HELPER_FUNCTIONS_HEADER