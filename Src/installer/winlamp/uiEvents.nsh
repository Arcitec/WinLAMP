!ifndef WINLAMP_PAGES_INSTALL_HEADER
!define WINLAMP_PAGES_INSTALL_HEADER

!include ".\ui.nsh"
!include "nx.nsh"

!ifdef EXPRESS_MODE
!define WINLAMP_LICENSE_AGREEMENT	${LICENSE_PATH}
!define WINLAMP_PRIVACY_POLICY		${PRIVACY_POLICY_PATH}
...
Function onXPClickLink
	...
	ExecShell open "http://www.winlamp.com/legal/eula/pc"
FunctionEnd
!endif
...
!ifdef EXPRESS_MODE
	${If} ${AtLeastWinVista}
		...
		${If} ${IsWinXP}
			IntOp $R2 $R2 - 16
			${NSD_CreateLink} 8u $R2 316u 16 "By clicking “Next”, you agree to the WinLAMP License Agreement."
...
Function UI_OnWelcomePageNotify
	...
	${OrIf} $R1 = ${NM_RETURN}
		...
		${If} $R3 == "winlamp_eula"
			StrCpy $R4 "http://www.winlamp.com/legal/eula/pc"
		${ElseIf} $R3 == "winlamp_privacy_policy"
			StrCpy $R4 "http://www.winlamp.com/legal/privacy"
		${Else}
			StrCpy $R4 ""
		${EndIf}
		...
		${If} $0 != "online"
			...
			${If} $R3 == "winlamp_eula"
				StrCpy $R4 "${WINLAMP_LICENSE_AGREEMENT}"
			${ElseIf} $R3 == "winlamp_privacy_policy"
				StrCpy $R4 "${WINLAMP_PRIVACY_POLICY}"
			${Else}
				StrCpy $R4 ""
			${EndIf}
			...
			${If} $R4 != ""
				...
				${IfNot} ${FileExists} "$R4"
					${If} $R3 == "winlamp_eula"
						ReserveFile "${WINLAMP_LICENSE_AGREEMENT}"
						File /oname=$R4 "${WINLAMP_LICENSE_AGREEMENT}"
					${ElseIf} $R3 == "winlamp_privacy_policy"
						ReserveFile "${WINLAMP_PRIVACY_POLICY}"
						File /oname=$R4 "${WINLAMP_PRIVACY_POLICY}"
					${EndIf}
				${EndIf}
				ExecShell open $R4
			${EndIf}
...
Function UI_OnFinishPageReadMe
	...
	System::Free $1
	WriteINIStr "$WINLAMPINI" "SETUP" "left" $2
	WriteINIStr "$WINLAMPINI" "SETUP" "top" $3
	WriteINIStr "$WINLAMPINI" "SETUP" "right" $4
	WriteINIStr "$WINLAMPINI" "SETUP" "bottom" $5
	...
	${AndIf} ${FileExists} "$INSTDIR\${WINLAMPEXE}"
		Push $0
		StrCpy $0 ""
		ClearErrors
		...
		${IfNot} ${Errors}
		${AndIf} $0 != ""
			ExecWait 'rundll32.exe $0,RunDll_ShellExecute "open" "$INSTDIR\${WINLAMPEXE}" "/NEW /REG=S" "$INSTDIR" 1' $1
			${If} ${Errors}
				StrCpy $1 1
			${EndIf}
		${EndIf}
	...
	${If} $1 != 0
		Exec "$INSTDIR\${WINLAMPEXE} /NEW /REG=S"
	${EndIf}
	Pop $1
