!ifndef NULLSOFT_WINLAMP_UNINSTALLER_PLAYER_HEADER
!define NULLSOFT_WINLAMP_UNINSTALLER_PLAYER_HEADER

!macro WALANG_UNINSTALL LNGPATH LANGID
	Delete "${LNGPATH}\WinLAMP-${LANGID}.wlz"
!macroend

!macro Uninstaller_KillWinLAMP
	...
	${Do}
		...
		${If} ${Errors}
			SetErrors
			IfSilent killWinLAMP_done
			MessageBox MB_DEFBUTTON2|MB_ABORTRETRYIGNORE "$(msgCloseWinLAMP)" IDABORT killWinLAMP_done IDRETRY +2
			ClearErrors
		${EndIf}
	${LoopWhile} ${Errors}
	...
	${Do}
		...
		${If} ${Errors}
			...
			${If} $0 = 0  ; something bad or other session
				IfSilent killWinLAMP_done
				MessageBox MB_DEFBUTTON2|MB_ABORTRETRYIGNORE "$(IDS_MSG_AGENTONOTHERSESSION)" IDABORT killWinLAMP_done IDRETRY +2
				ClearErrors
			${Else}
				IntOp $1 $1 + 1
				IntCmp $1 40 killWinLAMP_done
				SendMessage $0 ${WM_CLOSE} 0 0 /TIMEOUT=2000
				Sleep 250
				IsWindow $0 0 killWinLAMP_agentKilled
					SendMessage $0 ${WM_DESTROY} 0 0 /TIMEOUT=2000
					Sleep 250
				killWinLAMP_agentKilled:
			${EndIf}
		${EndIf}
	${LoopWhile} ${Errors}
...
Section "un.$(IDS_UNINSTALL_MEDIA_PLAYER)" IDX_UNINSTALL_MEDIA_PLAYER
	ExecWait '"$INSTDIR\${WINLAMPEXE}" /UNREG'
	!insertmacro 'Uninstaller_KillWinLAMP'
	...
	DeleteRegKey HKEY_CURRENT_USER 'Software\${WINLAMP}'

	DeleteRegKey HKEY_CLASSES_ROOT "${WINLAMP}.File"
	DeleteRegKey HKEY_CLASSES_ROOT "${WINLAMP}.Playlist"
	DeleteRegKey HKEY_CLASSES_ROOT "${WINLAMP}.SkinZip"
	DeleteRegKey HKEY_CLASSES_ROOT "${WINLAMP}.LangZip"
	DeleteRegKey HKEY_CLASSES_ROOT "${WINLAMP}.File.*"

	DeleteRegKey HKEY_CLASSES_ROOT "Directory\shell\${WINLAMP}.Play"
	DeleteRegKey HKEY_CLASSES_ROOT "Directory\shell\${WINLAMP}.Enqueue"
	DeleteRegKey HKEY_CLASSES_ROOT "Directory\shell\${WINLAMP}.Bookmark"
	DeleteRegKey HKEY_CLASSES_ROOT "Directory\shell\${WINLAMP}.WinLAMPLibrary"
	DeleteRegKey HKEY_CLASSES_ROOT "Directory\shell\${WINLAMP}.EnqueueAndPlay"
	DeleteRegKey HKLM "Software\Classes\Directory\shell\${WINLAMP}.Play"
	DeleteRegKey HKLM "Software\Classes\Directory\shell\${WINLAMP}.Enqueue"
	DeleteRegKey HKLM "Software\Classes\Directory\shell\${WINLAMP}.Bookmark"
	DeleteRegKey HKLM "Software\Classes\Directory\shell\${WINLAMP}.WinLAMPLibrary"
	DeleteRegKey HKLM "Software\Classes\Directory\shell\${WINLAMP}.EnqueueAndPlay"

	DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${WINLAMP}"
	DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\App Paths\${WINLAMPEXE}"
	...
	DeleteRegKey HKEY_CLASSES_ROOT "winlamp"
	; need to get "winlamp.exe /unreg" to do this instead...
	; DeleteRegValue HKEY_CLASSES_ROOT "pcast\shell\open\command" "" "$INSTDIR\${WINLAMPEXE} /HANDLE %1"
	; DeleteRegValue HKEY_CLASSES_ROOT "feed\shell\open\command" "" "$INSTDIR\${WINLAMPEXE} /HANDLE %1"
	...
	DeleteRegValue HKLM "Software\Microsoft\Windows\CurrentVersion\Explorer\AutoplayHandlers\EventHandlers\MTPMediaPlayerArrival" "${WINLAMP}MTPHandler"
	DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Explorer\AutoplayHandlers\Handlers\${WINLAMP}MTPHandler"
	DeleteRegKey HKEY_CLASSES_ROOT "Software\shell\${WINLAMP}MTPHandler"

	DeleteRegValue HKLM "Software\Microsoft\Windows\CurrentVersion\Explorer\AutoplayHandlers\EventHandlers\PlayMusicFilesOnArrival" "${WINLAMP}PlayMediaOnArrival"
	DeleteRegValue HKLM "Software\Microsoft\Windows\CurrentVersion\Explorer\AutoplayHandlers\EventHandlers\PlayCDAudioOnArrival" "${WINLAMP}PlayMediaOnArrival"
	DeleteRegValue HKLM "Software\Microsoft\Windows\CurrentVersion\Explorer\AutoplayHandlers\EventHandlers\PlayVideoFilesOnArrival" "${WINLAMP}PlayMediaOnArrival"
	DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Explorer\AutoplayHandlers\Handlers\${WINLAMP}PlayMediaOnArrival"

	DeleteRegKey HKLM "Software\Clients\Media\WinLAMP"
	DeleteRegValue HKLM "Software\RegisteredApplications" "WinLAMP"
...
!endif
	...
	Delete "$INSTDIR\WinLAMP.exe.manifest"
	Delete "$INSTDIR\WinLAMPmb.htm"
	Delete "$INSTDIR\whatsnew.txt"
	Delete "$INSTDIR\fmt.dll"
	Delete "$INSTDIR\winlamp.log"
	Delete "$INSTDIR\logs\winlamp.log"
	RMDir "$INSTDIR\logs"
	Delete "$INSTDIR\WinLAMP.lks"
	Delete "$INSTDIR\winlamp.pic"
	Delete "$INSTDIR\WinLAMP.q1"
	...
	${AndIfNot} ${Cmd} 'MessageBox MB_YESNO|MB_ICONQUESTION "$(msgRemoveMJUICE)" /SD IDYES IDNO'
		...
		DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\MJuiceWinLAMP" ; 12
	...
	Delete "$INSTDIR\Plugins\winlampFLV.swf"
	...
	RMdir /r "$INSTDIR\Plugins\avs\WinLAMP 5 Picks"
	...
	UnRegDLL "$INSTDIR\Plugins\Gracenote\cddbcontrolwinlamp.dll"
	UnRegDLL "$INSTDIR\Plugins\Gracenote\cddbuiwinlamp.dll"
	UnRegDLL "$INSTDIR\Plugins\Gracenote\CddbMusicIDWinLAMP.dll"
	UnRegDLL "$INSTDIR\Plugins\Gracenote\CddbPlaylist2WinLAMP.dll"
	Delete "$INSTDIR\Plugins\Gracenote\cddbcontrolwinlamp.dll"
	Delete "$INSTDIR\Plugins\Gracenote\cddbuiwinlamp.dll"
	Delete "$INSTDIR\Plugins\Gracenote\CddbWOManagerWinLAMP.dll"
	...
	Delete "$INSTDIR\Plugins\Gracenote\CddbMusicIDWinLAMP.dll"
	Delete "$INSTDIR\Plugins\Gracenote\CddbPlaylist2WinLAMP.dll"
	...
	; Languages
	ReadINIStr $0 "$WINLAMPINI" "WinLAMP" "LangDir"
	${If} $0 == ""
		StrCpy $0 "Lang"
	${EndIf}
	Push $INSTDIR
	Push $0
	nsis_winlamp::GetFullPath
	Pop $0
	...
	StrCpy $0 "$INSTDIR\${WINLAMPEXE}"
	ExecDos::exec /NOUNLOAD /TIMEOUT=5000 'netsh advfirewall firewall delete rule name="${WINLAMP}" program="$0"' "" ""
	Pop $1
	${If} $1 != 0
	...
	StrCpy $winlamp.uninstall.checkFolder "true"

SectionEnd

!endif ;NULLSOFT_WINLAMP_UNINSTALLER_PLAYER_HEADER