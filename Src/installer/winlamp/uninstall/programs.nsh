/*!ifndef NULLSOFT_WINLAMP_UNINSTALL_BUNDLE_HEADER
!define NULLSOFT_WINLAMP_UNINSTALL_BUNDLE_HEADER
...
SectionGroup "${INSTALL_TYPE_PREFIX}$(MUI_UNTEXT_CONFIRM_TITLE)" IDX_UNINSTALL_BUNDLES_GROUP
	Section "${INSTALL_TYPE_PREFIX}$(IDS_BUNDLE1_NAME)" IDX_UNINSTALL_WINLAMP_REMOTE
		${UninstallBundle_UninstallProgram} "${__SECTION__}" "Orb" "/S _?=$$INSTDIR" $0
	SectionEnd

	Section "${INSTALL_TYPE_PREFIX}$(IDS_BUNDLE2_NAME)" IDX_UNINSTALL_WINLAMP_TOOLBAR
		${UninstallBundle_UninstallProgram} "${__SECTION__}" "WinLAMP Toolbar" "/S _?=$$INSTDIR" $0
	SectionEnd

	Section "${INSTALL_TYPE_PREFIX}WinLAMP Detector Plug-in" IDX_UNINSTALL_BROWSER_PLUGIN
		${UninstallBundle_UninstallProgram} "${__SECTION__}" "WinLAMP Detect" "/S _?=$$INSTDIR" $0
	SectionEnd
...
Function ${INSTALL_TYPE_PREFIX}UninstallBundle_InitializeGroup
	${UninstallBundle_InitializeSection} ${IDX_UNINSTALL_WINLAMP_TOOLBAR} "WinLAMP Toolbar"
	${UninstallBundle_InitializeSection} ${IDX_UNINSTALL_BROWSER_PLUGIN} "WinLAMP Detect"
	${UninstallBundle_InitializeSection} ${IDX_UNINSTALL_WINLAMP_REMOTE} "Orb"
...
Function  ${INSTALL_TYPE_PREFIX}UninstallBundle_DescriptionProvider
	...
	Push $1
	${Select} $0
		${Case} ${IDX_UNINSTALL_WINLAMP_TOOLBAR}
			StrCpy $1 "WinLAMP Toolbar"
		${Case} ${IDX_UNINSTALL_BROWSER_PLUGIN}
			StrCpy $1 "WinLAMP Detect"
		${Case} ${IDX_UNINSTALL_WINLAMP_REMOTE}
			StrCpy $1 "Orb"
	...
	${If} $1 != ""
		...
		${If} $2 != ""
			${UninstallInfo_GetStringValue} $2 "winlampDescription" $1
			${If} $1 == ""
...
Function ${INSTALL_TYPE_PREFIX}UninstallBundle_UninstallProgram
	...
	${If} $3 != ""
		${UninstallInfo_GetExpandableStringValue} $3 "winlampUninstall" $0
		${If} $0 == ""
...
!endif ;NULLSOFT_WINLAMP_UNINSTALL_BUNDLE_HEADER*/