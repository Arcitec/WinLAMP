...
Function StartMenu_WriteData
	...
	${If} $1 != ""
		...
		${If} $0 == ""
			...
			CreateShortcut "$SMPROGRAMS\$1\${WINLAMPLINK}" "$INSTDIR\${WINLAMPEXE}"
			CreateShortcut "$SMPROGRAMS\$1\$(safeMode).lnk" "$INSTDIR\${WINLAMPEXE}" "/SAFE=1"
			CreateShortcut "$SMPROGRAMS\$1\$(uninstallWinLAMP).lnk" "$INSTDIR\uninstwa.exe"
		${EndIf}
	${EndIf}
  
	ReadIniStr $0 "$INSTINI" "Startmenu" "NoQuickLaunch"
	${If} $0 == ""
		CreateShortcut "$QUICKLAUNCH\${WINLAMPLINK}" "$INSTDIR\${WINLAMPEXE}"
	${EndIf}
	ReadIniStr $0 "$INSTINI" "Startmenu" "NoDesktop"
	${If} $0 == ""
		CreateShortcut "$DESKTOP\${WINLAMPLINK}" "$INSTDIR\${WINLAMPEXE}"
	${EndIf}
...
Function un.StartMenu_CleanData
	...
	${If} ${FileExists} "$0"
		...
		${If} $2 != "1"
			Delete "$QUICKLAUNCH\${WINLAMPLINK}"
		${EndIf}
		ReadIniStr $2 "$0" "Startmenu" "NoDesktop"
		${If} $2 != "1"
			Delete "$DESKTOP\${WINLAMPLINK}"
		${EndIf}
	${EndIf}
