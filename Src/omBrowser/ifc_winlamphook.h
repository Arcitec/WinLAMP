#ifndef NULLSOFT_WINLAMP_HOOK_INTERFACE_HEADER
#define NULLSOFT_WINLAMP_HOOK_INTERFACE_HEADER
...
static const GUID IFC_WinLAMPHook = 
{ 0xeb225b60, 0x14c1, 0x424f, { 0xb8, 0xea, 0xa8, 0xed, 0xe3, 0xad, 0x82, 0xa9 } };

#include <bfc/dispatch.h>

class __declspec(novtable) ifc_winlamphook: public Dispatchable
{
protected:
	ifc_winlamphook() {}
	~ifc_winlamphook() {}
...
inline HRESULT ifc_winlamphook::IsQuitAllowed(void)
{
	return _call(API_ISQUITALLOWED, (HRESULT)E_NOTIMPL);
}

inline HRESULT ifc_winlamphook::ResetFont(void)
{
	return _call(API_RESETFONT, (HRESULT)E_NOTIMPL);
}

inline HRESULT ifc_winlamphook::SkinChanging(void)
{
	return _call(API_SKINCHANGING, (HRESULT)E_NOTIMPL);
}

inline HRESULT ifc_winlamphook::SkinChanged(const wchar_t *skinName)
{
	return _call(API_SKINCHANGED, (HRESULT)E_NOTIMPL, skinName);
}

inline HRESULT ifc_winlamphook::FileMetaChange(const wchar_t *fileName)
{
	return _call(API_FILEMETACHANGE, (HRESULT)E_NOTIMPL, fileName);
}

inline HRESULT ifc_winlamphook::SysColorChange(void)
{
	return _call(API_SYSCOLORCHANGE, (HRESULT)E_NOTIMPL);
}

inline HRESULT ifc_winlamphook::SkinColorChange(const wchar_t *colorTheme)
{
	return _call(API_SKINCOLORCHANGE, (HRESULT)E_NOTIMPL, colorTheme);
}

#endif // NULLSOFT_WINLAMP_HOOK_INTERFACE_HEADER