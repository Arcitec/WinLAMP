...
**
** genhotkeys_add_ipc=SendMessage(winlampWindow,WM_WA_IPC,(WPARAM)&"GenHotkeysAdd",IPC_REGISTER_WINLAMP_IPCMESSAGE);
**
** Then you can use:
**
** PostMessage(winlampWindow,WM_WA_IPC,(WPARAM)&myGenHotkeysAddStruct,genhotkeys_add_ipc);
*/
...
#define HKF_HWND_WPARAM 0x2     // sets wParam with WinLAMP's window handle
...
#define HKF_SHOWHIDE 0x20       // brings WinLAMP to front or minimizes WinLAMP if already at front
#define HKF_NOSENDMSG 0x40      // don't send any message to the winlamp window
...
typedef struct {
  ...
  UINT uMsg;      //message that will be sent to winlamp's main window (must always be !=NULL)
  WPARAM wParam;  //wParam that will be sent to winlamp's main window
  LPARAM lParam;  //lParam that will be sent to winlamp's main window
  char *id;       //unique string to identify this command - case insensitive
  HWND wnd;       //set the HWND to send message (or 0 for main winlamp window)
...
**   hotkey->name = name;
**   hotkey->id = id;
**   *ipc = hotkey->uMsg = SendMessage(winlampWindow,WM_WA_IPC,(WPARAM)&hotkey->id,IPC_REGISTER_WINLAMP_IPCMESSAGE);
**
**   genhotkeys_add_ipc=SendMessage(winlampWindow,WM_WA_IPC,(WPARAM)&"GenHotkeysAdd",IPC_REGISTER_WINLAMP_IPCMESSAGE);
**   PostMessage(winlampWindow,WM_WA_IPC,(WPARAM)hotkey,genhotkeys_add_ipc);
** }
*/
...
**
** LRESULT CALLBACK WinLAMPWnd(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam){
** int ret = 0;
**
...
**   }
**
**   // do stuff before the winlamp proc has been called
** 
**   ret = CallWindowProc(WinLAMPProc,hwnd,uMsg,wParam,lParam);
**
**   // do other stuff after the winlamp proc has been called
