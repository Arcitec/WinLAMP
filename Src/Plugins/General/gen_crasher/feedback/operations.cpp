...
{
	static wchar_t subject_str[512] = {L"WinLAMP Error Report"};
	...
	if (GetModuleFileName(0, path, MAX_PATH))
	{
		...
		if (p) *p = 0;
		PathAppend(path, L"winlamp.exe");

		HKEY hkey = NULL;
		if (RegCreateKeyEx(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Nullsoft\\WinLAMP", 0, 0, 0, KEY_READ, NULL, &hkey, NULL) == ERROR_SUCCESS)
		{
...
{
	BOOL retCode = FALSE;
	const wchar_t *subject = L"WinLAMP Error Report";
	const wchar_t *senderName = L"WinLAMP Error Reporter";
	const wchar_t *recipientAddress = L"bug@winlamp.com";
	...
	if (settings.sendBySMTP)
	{
		...
		body = L"This message was generated by WinLAMP Error Reporter v1.09.\r\nPlease check attachments for viruses.\r\n";
		body.Data.append(L"\r\n");
...
BOOL Restart(void)
{
	...
	return  CreateProcess(
						settings.ReadWinLAMP(),	// name of executable module
