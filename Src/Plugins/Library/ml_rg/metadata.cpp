...
int GetFileInfo(const wchar_t *filename, const wchar_t *metadata, wchar_t *dest, int len)
{
	...
	return (int)(INT_PTR)SendMessage(plugin.hwndWinLAMPParent, WM_WA_IPC, (WPARAM)&efis, IPC_GET_EXTENDED_FILE_INFOW_HOOKABLE); //will return 1 if wa2 supports this IPC call
}

void TagUpdated(const wchar_t *filename)
{
	SendMessage(plugin.hwndWinLAMPParent, WM_WA_IPC, (WPARAM)filename, IPC_FILE_TAG_MAY_HAVE_UPDATEDW);
}