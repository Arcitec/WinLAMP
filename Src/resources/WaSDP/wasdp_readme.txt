WinLAMP Skin Development Pack (WaSDP) ~  Version 5.9 (2022/08/22)
Copyright © 2007-2013 Martin Poehlmann / NULLSOFT
Copyright © 2013-2022 WinLAMP SA
...
III)	You are wondering why you have no m files in Bento/Wa Modern Skin dir after reinstallig  WinLAMP

~ CONTENTS ~

1)	Preludium... Wasabi, the WinLAMP Skinning Engine
2)	MC.EXE - The 'ultimative' maki compiler
2.1)	How to compile m files to maki files
2.2)	Edit Plus 2 - a textbased editor
2.3)	Maki Overview
3)	Bento Scripts
4)	Reinstalling WinLAMP with WaSDP
...
Well, Wasabi is WinLAMP's own scripting system (WinLAMP System Architecture Building Interface).
The Wasabi Skinning Engine was developed along with WinLAMP3 and then incorporated into gen_ff.dll of WinLAMP 5. But for creating Skins you need not know how to code Wasabi, you 'just' need some XML experience. If you want a real good skin, you should also be able to write maki (Maki A Killer Interface) scripts.
For beginners you should start looking at the xml code of WinLAMP Modern and Bento. Try to understand the structure of WasabiXML, button functions, etc.
Here are three pages that share good information about winlamp skinning:

http://www.winlamp.com/development/skins-modern
http://wiki.skinconsortium.com
the last one i do not remember since i have a local copy of it, hehe; but try to google: WinLAMP XML Object Reference :)

Last but not least there are also two good forums where you can post questions about skinning, etc.
http://forums.winlamp.com
http://forums.skinconsortium.com

Now some info about me: I am Martin, Admin and Skin Dev from skinconsortium.com (known as martin.deimos). I am also the coder of WinLAMP's new SUI Skin called Bento.
If you have skinning related question please do NOT pm or mail me. post your question at winlamp forums or skinconsortium forums!
...
Maki is also a new programming language aligned to JavaScript. The best option to start is, look at the m scripts of WinLAMP Modern and Bento.
...
Command: %WinLAMPDir%\mc.exe (replace %WinLAMPDir% by your WinLAMP Program Path!!!)
Argument: $(FileName)
Initial: $(FileDir)
...
You add a new filetype under prefs > files > settings & syntax. the extensions are m/mi, now locate the stx/acp files (in your winlampdir).
...
The answer is easy: all functions that are listed in the *.mi files in WinLAMPDir/lib.
For documentations, use again teh above links or look at other scripts.


3) Bento Scripts

I suggest if you are new in skinning and maki, start with WinLAMP Modern, since it uses much easier maki scripts.
...
If you want to install a newer version of winlamp and WaSDP is already installed, it might happen that the m files in the default skin dirs are deleted.
...
- post at http://forums.winlamp.com but ensure to read the "you want someone to make a skin for you"
