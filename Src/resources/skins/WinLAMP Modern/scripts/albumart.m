...
{
	initAttribs();
	Container albumart = System.getContainer("winlamp.albumart");
...
{
	if (!aalayout) return;
	setPrivateInt("WinLAMP Modern", "Album Art XPos", aalayout.getLeft());
	setPrivateInt("WinLAMP Modern", "Album Art YPos", aalayout.getTop());
}

aalayout.onStartup ()
{
	resize(getPrivateInt("WinLAMP Modern", "Album Art XPos", 0), getPrivateInt("WinLAMP Modern", "Album Art YPos", 0), getWidth(), getHeight());
}
