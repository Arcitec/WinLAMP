...
System.onScriptLoaded() {
	...
	mychange = 1;
	setTabs(getPrivateInt("winlamp5", "ConfigTab", 1));
	mychange = 0;
	if (getPrivateInt("winlamp5", "DrawerOpen", 0)) {
		OpenDrawer(0);
		adjustSnapPoints(1);
	} else {
		adjustSnapPoints(0);
...
setTabs(int tabstate) {
	...
	setPrivateInt("winlamp5", "ConfigTab", tabstate);
	updateAttribs();
}
...
OpenDrawer(int animate) {
	...
	if (animate && scrollconfigdrawerattrib.getData() == "1") {
		...
		setPrivateInt("winlamp5", "DrawerOpen", 1);
		ColorThemes.show();
		adjustSnapPoints(1);
		updateAttribs();
		main.Redock();
...
//	main.setXmlParam("minimum_h", "280");
	...
	if (animate && scrollconfigdrawerattrib.getData() == "1") {
		...
		DrawerShadow.hide();
		setPrivateInt("winlamp5", "DrawerOpen", 0);
		adjustSnapPoints(0);
		updateAttribs();
		main.redock();
	}
...
menubar_main_attrib.onDataChanged() {
	...
	main.beforeRedock();
	int DrawerOpen=getPrivateInt("winlamp5", "DrawerOpen", 0);
	if (DrawerOpen) {
		main.snapAdjust(0,0,0,0+menuHeight);
	} else {
		main.snapAdjust(0,0,0,116+menuHeight);
