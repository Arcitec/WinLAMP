...
#define WINLAMPBANDS "70 Hz,180 Hz,320 Hz,600 Hz,1 KHz,3 KHz,6 KHz,12 KHz,14 KHz,16 KHz"
...
System.onScriptLoaded()
{
	...
	WinLAMPConfigGroup eqwcg = WinLAMPConfig.getGroup("{72409F84-BAF1-4448-8211-D84A30A1591A}");

	int freqmode = eqwcg.getInt("frequencies"); // returns 0 for classical winlamp levels, 1 for ISO levels
...
System.onEqFreqChanged (boolean isoonoff)
{
	...
	{
		frequencyLabel.setXmlParam("image", "equalizer.labels.winlamp");
		for(int i=0; i<10; i++) frameGroup.findObject("eq"+integerToString(i+1)).setXmlParam("tooltip", getToken(WINLAMPBANDS,",",i));
	}
}
...
system.onEqBandChanged(int band, int value)
{
	...
	if (eqwcg.getInt("frequencies") == 1) {
		...
		if (f >= 0) t = "EQ: " + translate(getToken(WINLAMPBANDS,",",band)) + ": +" + floattostring(f, 1) + " "+ translate("dB");
		else t = "EQ: " + translate(getToken(WINLAMPBANDS,",",band)) + ": " + floattostring(f, 1) + " "+ translate("dB");
	}
