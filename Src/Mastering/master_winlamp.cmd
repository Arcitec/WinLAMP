...
title Nullsoft WinLAMP build script -= %1 =-
...
SET INSTALLER_LANG=Mastering\WinLAMP\installer_%BUILDTYPE%.lang
SET INSTALLER_CONFIG=Mastering\WinLAMP\installer_%BUILDTYPE%.config
...
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\master_winlamp_%BUILDTYPE%.xml" >nul
if errorlevel 1 goto CVS_ERROR_1
goto master86
:master64
echo using x64 mastering script
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\master_winlamp_%BUILDTYPE%_x64.xml" >nul
if errorlevel 1 goto CVS_ERROR_1
:master86
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\build_winlamp_%BUILDTYPE%.xml" >nul
if errorlevel 1 goto CVS_ERROR_2
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\configmap.xml"  >nul
if errorlevel 1 goto CVS_ERROR_3
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\version_%BUILDTYPE%.info"  >nul
...
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\constants.h" >nul
if errorlevel 1 goto CVS_ERROR_6
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\buildnumber.h" >nul
if errorlevel 1 goto CVS_ERROR_7
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\talkback.ini" >nul
if errorlevel 1 goto CVS_ERROR_7
if NOT %1==NIGHT cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\set_tags_%BUILDTYPE%.cmd" >nul
if errorlevel 1 goto CVS_ERROR_8
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\fileNames.cmd" >nul
if errorlevel 1 goto CVS_ERROR_9
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\build_installer.cmd" >nul
if errorlevel 1 goto CVS_ERROR_10
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\build_wbm.cmd" >nul
if errorlevel 1 goto CVS_ERROR_10
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\rebase.cmd" >nul
if errorlevel 1 goto CVS_ERROR_10
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\build_webdev.cmd" >nul
if errorlevel 1 goto CVS_ERROR_10
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\build_orb.cmd" >nul
if errorlevel 1 goto CVS_ERROR_10
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\build_wadetect.cmd" >nul
if errorlevel 1 goto CVS_ERROR_10
...
cvs checkout  -N -d "%CURSANDBOX%"   "Mastering\WinLAMP\simple_sign.cmd" >nul

f:
cd \
echo Patching version number...
cd "%CURSANDBOX%\Mastering\WinLAMP"
...
if NOT %1==NIGHT call "%CURSANDBOX%\Mastering\WinLAMP\set_tags_%BUILDTYPE%.cmd" 
if errorlevel 1 goto SET_TAGS_ERROR

SET GIT_ASK_YESNO=false
git clone -b "%REPLICANT_GIT_BRANCH%" ssh://winlamp@git.cm.aol.com/nullsoft/replicant.git "%CURSANDBOX%\replicant"
if errorlevel 1 goto GIT_ERROR

echo Setting Installer names...
call "%CURSANDBOX%\Mastering\WinLAMP\fileNames.cmd"
...
perl -S dgbuild.pl master_winlamp_%BUILDTYPE%.xml > nul
goto build86
:build64
perl -S dgbuild.pl master_winlamp_%BUILDTYPE%_x64.xml > nul
:build86
...
:CVS_ERROR_1
echo Error!!! Unable to checkout 'Mastering/WinLAMP/master_winlamp.xml'
goto END

:CVS_ERROR_2
echo Error!!! Unable to checkout 'Mastering/WinLAMP/build_winlamp.xml'
goto END

:CVS_ERROR_3
echo Error!!! Unable to checkout 'Mastering/WinLAMP/configmap.xml'
goto END

:CVS_ERROR_4
echo Error!!! Unable to checkout 'Mastering/WinLAMP/version.info'
goto END

:CVS_ERROR_5
echo Error!!! Unable to checkout 'Mastering/WinLAMP/verctrl.exe'
goto END

:CVS_ERROR_6
echo Error!!! Unable to checkout 'Mastering/WinLAMP/constants.h'
goto END

:CVS_ERROR_7
echo Error!!! Unable to checkout 'Mastering/WinLAMP/buildnumber.h'
goto END

:CVS_ERROR_8
echo Error!!! Unable to checkout 'Mastering/WinLAMP/set_tags_%BUILDTYPE%.cmd'
goto END

:CVS_ERROR_9
echo Error!!! Unable to checkout 'Mastering/WinLAMP/fileNames.cmd'
goto END
:CVS_ERROR_10
echo Error!!! Unable to checkout 'Mastering/WinLAMP/build_installer.cmd'
goto END
:CVS_ERROR_11
echo Error!!! Unable to checkout 'Mastering/WinLAMP/%INSTALLER_LANG%'
goto END
:CVS_ERROR_12
echo Error!!! Unable to checkout 'Mastering/WinLAMP/%INSTALLER_CONFIG%'
goto END

:CVS_ERROR_COMMIT
echo Error!!! Unable to commit 'Mastering/WinLAMP/constants.h'
goto END
